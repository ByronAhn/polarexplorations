...
\midrule \endhead
™™™™{*} & Prominence & A syllable that has intonational prominence \tabularnewline
™™™™{]} & Phrase’s Right Edge & The right edge of the final word of a phrase \tabularnewline
\bottomrule 
...
Similar to uncertainty labels for prominence, a labeller can indicate uncertainty about the presence of a phrase boundary using the ‘™™™™{?]}’ label. For example, in \texttt{librivox-zip1}, the labeller is uncertain of whether there is a phrase boundary between “®®®®{Librivox}” and “®®®®{recording}”:
...
(Note that there is also a ™™™™{?*} on the final syllable of “®®®®{Librivox}”.) Again, additional cases of labeller uncertainty are provided in chapter \ref{ch:advanced}.
...
\midrule \endhead
™™™™{*} & Prominence & A syllable that has intonational prominence \tabularnewline
™™™™{?*} & Possible Prominence & A syllable that might have intonational prominence \tabularnewline
™™™™{]} & Phrase’s Right Edge & The right edge of the final word of a phrase \tabularnewline
™™™™{?]} & Possible Phrase’s Right Edge & What might be the right edge of the final word of a phrase \tabularnewline
\bottomrule 
...
PoLaR allows two levels of annotation detail on the Points Tier: the default (‘™™™™{0}’) label is described in this chapter. (Advanced labels for associating the pitch turning points on the Points tier with the prosodic events on the PrStr tier, are described in Chapter \ref{ch:advanced}.)
...
\caption{The f0 contour for \texttt{amelia\_knew2}, with the Points tier annotated with Basic ‘™™™™{0}’ labels.%
...
\caption{\texttt{out\_of\_order-plateau}, with the Points tier annotated with ‘™™™™{0}’ labels.%
...
\caption{\texttt{really\_would\_not\_annoy}, with the Points tier annotated with ‘™™™™{0}’ labels.%
...
\midrule \endhead
™™™™{0} & A turning point in the f0 contour (where line segments meet in a straight-line approximation of the pitch) \tabularnewline
™™™™{?0} & Labeller is uncertain whether it is necessary to annotate a turning point in the f0 contour, even after exploring options with the straight line approximation resynthesis tool \tabularnewline
™™™™{0,X} & An f0 turning point whose pitch value is approximately X (a labeller’s approximation of the f0 when the software-based f0 tracking is unreliable) \tabularnewline
\bottomrule 
...
Viewing the entire utterance as a whole, one can perceive it as having three different \uline{local pitch ranges}, \textbf{each of which is labelled as an interval on the “Ranges” tier}. Consider the high and low pitch in “®®®®{okay so this is old advice}” compared to the highs and lows in “®®®®{but is it necessarily bad advice}”. Although the highs and lows in “®®®®{okay\ldots advice}” are much lower than in “®®®®{but is it\ldots advice}”, on their own, they can be perceived as \uline{locally} high (even though, e.g., “®®®®{okay}” is much lower than the later highs) and low. Similarly, these highs and lows are also different from the highs and lows in “®®®®{that keeps on getting recycled through the ages}”.
...
If the labeller has access to PrStr labels, they may find it useful to consider ranges with respect to phrases as delimited by the ™™™™{]} label. At the same time, domains for ranges need not be determined by phrases labelled in the PrStr tier. Even a short stretch of speech may well turn out to contain local pitch range changes, as described in the Advanced labels chapter (Ch.\ref{ch:advanced}). Conversely, sequential phrases may be perceived as part of the same range. Ranges may to some extent suggest groupings among phrases, such as those labelled with ™™™™{]} in the PrStr tier. In this way, PoLaR Ranges annotation gives the labeller the means to capture more complicated relations where phrase boundaries (as annotated via the PrStr tier) and ranges can be somewhat independent: ranges can be annotated delinked from other boundary cues.
...
At the same time, the labeller should be a little cautious, and give a little wiggle room from the measured f0 min\slash max (rounding up or down to a number ending in 5 or 0, typically by at least 1 or 2 Hz, and often by a few more), since precise numbers are influenced by a number of factors, including software settings. Thus, a measured f0 range of 112.4Hz to 273.2Hz should have the min rounded down and max rounded up, and then be annotated as an interval in the Ranges tier with a label like ‘™™™™{110-280}’. In other words, \textbf{in the typical case, (rounded) measures of local f0 min and local f0 max can be used to determine the label for a Ranges interval}. (This is the default guidance; more complex cases will be discussed here and in section \ref{sec:ranges-advanced} of Ch.\ref{ch:advanced}.)
...
Conceptually, the Levels tier divides the pitch space within given range (taken from the Range tier) into 5 equal bands or sub-ranges, as schematized in Figure \ref{fig:tomorrow_morning Levels basic}. In this example, the first point (falling during the [m] of “®®®®{tomorrow}”) corresponds to a turning point whose f0 in the pitch track is located in the top band (shown in red). Accordingly, this point gets assigned a 5 in the levels tier. Likewise the second point, which while slightly higher in f0, is still within the red band representing level 5. The following point, later in the [a] of “®®®®{tomorrow}”, corresponds to a slightly lower f0 that falls within the 4 band (shown in yellow), and therefore gets assigned a level 4. Points marked at both the end of “®®®®{tomorrow}” and the end of “®®®®{morning}” correspond to f0 points in the lowest band (in purple), and are assigned level 1. (Note that in this particular example, there is a point where the pitch tracker is unreliable during “®®®®{tomorrow}”, as the speaker’s voice has become creaky. Recall section \ref{sec:optional-f0-override-labels-for-annotating-pitch-points-without-a-reliable-f0-track}).
...
For example, the file \texttt{no\_cigar}, shown in Figure \ref{fig:no_cigar Levels basic}, has 2 ranges annotated: 1) from 125-175 Hz for the word “®®®®{close}”, and 2) from 110-145 Hz for the word sequence “®®®®{but no cigar}”. Note that the pitch-accent-related point in the “®®®®{-gar}” of “®®®®{cigar}” is at around 140 Hz, so corresponds to a High in the range for “®®®®{but no cigar}” (as reflected in the level 5 designation, visually represented as the highest colored band in the right side of the figure), but it is only a bit higher (in absolute f0) than the point in “®®®®{close}” that is at 130 Hz, so corresponds to a Low (as reflected by the level 1 designation, visually represented by the lowest colored band in the left side of the figure) in the range for that word.
...
The example in figure \ref{fig:design improvements f0-tracking} shows additional examples where the f0 track is heavily distorted by non-sonorant sounds.  In particular, note that the [z] in “®®®®{design}” and the [mp] in “®®®®{improvements}” both cause a significant drops in the f0 track, but they do not correspond to drops in pitch – listening to this example, labellers perceive the pitch as rising steadily from the beginning of “®®®®{design}” to the peak in “®®®®{improvements}”, as reflected by the Points tier labels (which skip over the two mentioned drops in f0).

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Appendix-design_improvements.png}
\caption{An example where non-sonorant sounds disturb the f0 tracking; this is especially noticeable in the [z] of “®®®®{design}” and [p] of “®®®®{improvements}”.%
\label{fig:design improvements f0-tracking}%
\index{Annotated example, f0 tracking!design\_improvements}%
}
\end{figure}

Consider now the example in figure \ref{fig:out of order-plateau f0-tracking}, which exhibits many small rises\slash falls in the f0 track. Note that the Points tier annotations reflect that the annotators perceive the pitch containing many fewer turning points – for example, the Points annotation reflects a mostly steady pitch during “®®®®{seems like it’s}”. This perception of much smoother pitch is reflected in the straight line approximation resynthesis in figure \ref{fig:out of order-plateau f0-tracking}.
...
A particularly frequent non-modal phonation is creaky voice; it is especially common at the ends of phrases and where the pitch is low. It is marked by irregular pitch periods and especially perceptual glottalization (which can be seen in a spectrogram as prominent vertical striations, as during the bulk of the final [i] in Fig. \ref{fig:joey f0-tracking}). During creaky voice, the f0 may disappear or appear to jump suddenly upwards\slash downwards. Both of these effects can be seen in Fig. \ref{fig:joey f0-tracking}: there is creaky voice during “®®®®{it to}” and at the end of “®®®®{Joey}”; during “®®®®{it to}” there is no f0 tracking, and during “®®®®{Joey}”, the f0 tracking jumps suddenly upwards (at the beginning of [i]) and then disappears until the speaker returns to modal voice at the end of the utterance.
...
\caption{The labeller hears the phrase boundary between “®®®®{obvious}” and “®®®®{educate}” as stronger than the phrase boundary between “®®®®{first}” and “®®®®{seems}”.%
...
\caption[The labeller has annotated the beginning of each phrase as well as its end.]{The labeller has annotated the beginning of each phrase as well as its end, including an instance of the same location being both a phrase end and a phrase beginning, i.e. the marker between “®®®®{beans}” and “®®®®{some}.”%
\label{fig:vegetable_list PrStr Adv}%
\index{Annotated example, PrStr tier (advanced)!vegetable\_list}
}
\end{figure}

Note that we offer this ™™™™{[} label as a tool for those who would find using it to be helpful for their research purposes, and using it is by no means required. In other words, use the ™™™™{[} label if you are compelled to, but we take no stance on how often labellers should be using it (if at all). For some cases where ™™™™{[} labels may be useful for Advanced Points labels, see section \ref{sec:points-associated-with-phrase-initial-boundaries}.
...
\midrule \endhead
\rowcolor{green}
™™™™{]} & Prosodic Phrase Boundary & A prosodic phrase ends at this point \tabularnewline
™™™™{?]} & Possible Prosodic Phrase Boundary & A prosodic phrase may end at this point, but the labeller is uncertain \tabularnewline
™™™™{]]} & Large Prosodic Phrase Boundary & A noticeably large prosodic phrase ends at this point \tabularnewline
™™™™{[} & Prosodic Phrase Boundary & A prosodic phrase begins at this point \tabularnewline
™™™™{?[} & Possible Prosodic Phrase Boundary & A prosodic phrase may begin at this point, but the labeller is uncertain \tabularnewline
™™™™{[[} & Large Prosodic Phrase Boundary & A noticeably large prosodic phrase begins at this point \tabularnewline
\bottomrule
...
\caption{The labeller is annotating the perception that “®®®®{Naomi}” and “®®®®{email}” are both prominent, but that “®®®®{Naomi}” is significantly more prominent.%
...
\midrule \endhead
\rowcolor{green}
™™™™{*} & Prominence & The constituent this occurs in is prominent\tabularnewline
™™™™{?*} & Possible Prominence & The constituent this occurs in might be prominent, but the labeller is uncertain \tabularnewline
™™™™{**} & Especially Strong Prominence & The constituent this occurs in is especially prominent \tabularnewline
\bottomrule
...
While disfluencies may differ prosodically from one another, this ‘d’ label can be used for all types. An example of a disfluency that corresponds to a momentary stutter (with no sense of disjuncture) is given in Figure \ref{fig:in_a_plane-disfluencies PrStr Adv}. Additionally, an example of a disfluency that induces a sense of disjuncture like that of a phrase boundary (due to the overlap in set of cues) is given in Figure \ref{fig:spoon_here PrStr Adv}. In both cases, the ‘™™™™{d}’ label is on the PrStr tier, time-aligned with the onset of the disfluent moment\slash region, or where the speaker cuts themselves off.
...
\centering
\includegraphics[width=.875\linewidth]{PrStr-in_a_plane-disfluencies-adv.png}
\caption[\texttt{spoon\_here}, with Advanced PoLaR labels. Note the ‘™™™™{d}’ on PrStr.]{\texttt{spoon\_here}, with Advanced PoLaR labels. Note the ‘™™™™{d}’ on PrStr, annotating where a labeller may perceive some of the prosody associated with this disfluency as cues to phrase break.%
...
\caption[\texttt{experience}, with Advanced PoLaR labels. Note the region flanked by ‘™™™™{\{d}’ and ‘™™™™{d\}}’ on PrStr.]{\texttt{experience}, with Advanced PoLaR labels. Note the region flanked by ‘™™™™{\{d}’ and ‘™™™™{d\}}’ on PrStr, which annotate a stretch of time that sounds disfluent.%
...
\begin{longtable}{cp{.3\linewidth}p{.45\linewidth}}
	...
	\midrule
	\endhead
	™™™™{d} & Disfluency & A point in the utterance where there is a disfluency\tabularnewline
	™™™™{\{d} & Start of a disfluent region & A point in the utterance where a disfluent region of speech begins\tabularnewline
	™™™™{d\}} & End of a disfluent region & A point in the utterance where a disfluent region of speech ends\tabularnewline
	\bottomrule
...
In this section we have reviewed some ways to systematically label these more refined intuitions, beyond the simple contrast of presence\slash absence of ™™™™{*} and ™™™™{]} labels; e.g., using the contrast between ™™™™{*} and ™™™™{**} labels, and ™™™™{]} and ™™™™{]]} labels to differentiate degrees of prominence\slash phrasing. In addition to the examples provided earlier in this section, these more fine-grained contrasts are exemplified in Figure \ref{fig:out_of_order_huh PrStr Adv} and Figure \ref{fig:asked_only_an_illusion PrStr Adv}.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-out_of_order_huh-adv.png}
\caption[The labeller hears especially strong breaks after “®®®®{huh}” and after “®®®®{again}”.]{The labeller hears especially strong breaks after “®®®®{huh}” and after “®®®®{again}”; they also hear especially strong prominence on “®®®®{order}”.%
...
\caption[The labeller is at several points uncertain about prominence.]{The labeller is at several points uncertain about prominence (“®®®®{she}”, “®®®®{believes}”, “®®®®{only}”), while also hearing especially strong prominence on “®®®®{illusion}” and especially strong phrase boundaries after “®®®®{still}” and “®®®®{illusion}”.%
\label{fig:asked_only_an_illusion PrStr Adv}%
\index{Annotated example, PrStr tier (advanced)!asked\_only\_an\_illusion}
}
\end{figure}

The following example Figure \ref{fig:at_this_point-disfluencies PrStr Adv} includes instances of both labeller uncertainty about prominence (™™™™{?*}) and a perceived a disfluency (marked with the ‘™™™™{d}’ label). Note that some disfluencies have pauses immediately following and some do not; if it feels like the disfluency leads to new prosodic phrase boundaries, these boundaries should be annotated separately. (Likewise for disfluency-induced prominence.)

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-at_this_point-disfluencies-adv.png}
\caption[The labeller hears several disfluencies.]{The labeller hears several disfluencies, where the speaker restarts\slash recasts their speech; the ‘™™™™{d}’ labels that indicate this are at the cut-off point, before any existing disfluency-related pauses.%
...
\begin{longtable}{cp{.3\linewidth}p{.45\linewidth}}
	...
	\endhead
	\rowcolor{green}
	™™™™{]} & Prosodic Phrase Boundary & The end of a Words interval where a prosodic phrase ends \tabularnewline
	™™™™{?]} & Possible Prosodic Phrase Boundary & The end of a Words interval where a prosodic phrase may end (but the labeller is uncertain) \tabularnewline
	™™™™{]]} & Large Prosodic Phrase Boundary & The end of a Words interval where a noticeably large prosodic phrase ends \tabularnewline
	™™™™{[} & Prosodic Phrase Boundary & The beginning of a Words interval where a prosodic phrase begins \tabularnewline
	™™™™{?[} & Possible Prosodic Phrase Boundary & The beginning of a Words interval where a prosodic phrase may begin (but the labeller is uncertain) \tabularnewline
	™™™™{[[} & Large Prosodic Phrase Boundary & The beginning of a Words interval where a noticeably large prosodic phrase \tabularnewline
	\hline
	\rowcolor{green}
	™™™™{*} & Prominence & The center of the Phones/Words interval that is prominent\tabularnewline
	™™™™{?*} & Possible Prominence & The center of the Phones/Words interval that might be prominent (but the labeller is uncertain) \tabularnewline
	™™™™{**} & Especially Strong Prominence & The center of the Phones/Words interval that is especially prominent \tabularnewline
	\hline
	™™™™{d} & Disfluency & A point in the utterance where there is a disfluency\tabularnewline
	™™™™{\{d} & Start of a disfluent region & A point in the utterance where a disfluent region of speech begins\tabularnewline
	™™™™{d\}} & End of a disfluent region & A point in the utterance where a disfluent region of speech ends\tabularnewline
...
\item A particular \textit{\uline{type}} of PrStr labelled event (either ‘™™™™{*}’ or ‘™™™™{]}’; marked below as ‘™™™™{\_}’) is associated with the f0 turning point, which is time-aligned in various ways, indicated by one of three \textit{\uline{pointer}}s:

 \begin{enumerate} \def\labelenumii{\alph{enumii}.}
\item PrStr label follows Points label (‘™™™™{\_>}’)
\item PrStr label precedes Points label (‘™™™™{\_<}’)
\item {[\textit{uncommon}]} PrStr label is precisely co-timed with Points label (‘™™™™{\_@}’) \end{enumerate}
\item Occasionally, multiple PrStr labelled events are associated with a single f0 turning point. That is, the turning point is either (i) ambiguous between being associated with the preceding PrStr labelled event and being associated with the following event, or (ii) the labeller believes that the point relates to both of those events. In these cases a complex label is used, with two labels separated by a ™™™™{/} (‘™™™™{\_</\_>}’). \end{enumerate}
...
\centering
  \begin{tikzpicture}[node distance=4em, anchor=west, align=flush center, FLOW/.style={->, very thick}, LABEL/.style={draw, fill=yellow!80, inner sep=.75em}, scale = .775, transform shape]
    ...
    \node[LABEL, below right=1em and 6em of node2.east] (node3no) {™™™™{0}};
    \node[LABEL, above right=1em and 6em of node2.east] (node3yes) {™™™™{*}\\™™™™{]}\\™™™™{[}};
    ...
    \node[LABEL, right=3em of node4.east] (node5) {™™™™{>}\\™™™™{<}\\™™™™{@}};
    ...
    \node[LABEL, above=of node7yes.center] (node7) {™™™™{/}};
...
In summary, these Advanced Points tier labels are either (1) the simple ‘™™™™{0}’ label, (2) a complex label with two components in the order \textit{<{type}>{}} \textit{<{pointer}>}, or (3) a label made up of two complex labels separated by a ™™™™{/}.
...
™™™™{0} & The default label is still the most appropriate choice if the annotator chooses not to affiliate a turning point with a Prosodic Structure label. (No pointer is used in conjunction with a ‘™™™™{0}’ label.) \tabularnewline\hdashline
™™™™{*} & An f0 turning point for a ™™™™{*} (\textit{of any type; e.g., also ™™™™{**} or ™™™™{?*}}) in the Prosodic Structure tier \tabularnewline\hdashline
™™™™{]} & An f0 turning point for a ™™™™{]} (\textit{also ™™™™{?]} or ™™™™{]]}}) in the Prosodic Structure tier \tabularnewline\hdashline
™™™™{[} & An f0 turning point for a ™™™™{[} (\textit{also ™™™™{?[} or ™™™™{[[}}) in the Prosodic Structure tier \tabularnewline
\midrule
\textbf{Pointer Label} & \textbf{Timing of the relevant Prosodic Structure Tier Object}\tabularnewline\hdashline
™™™™{>{}} & This f0 turning point is related to a Prosodic Structure tier object that follows it \tabularnewline\hdashline
™™™™{<{}} & This f0 turning point is related to a Prosodic Structure tier object that precedes it \tabularnewline\hdashline
™™™™{@} & This f0 turning point is related to a Prosodic Structure tier object that is precisely aligned with it \tabularnewline
\midrule
\textbf{Special Label} & \textbf{Noted ambiguity} \tabularnewline\hdashline
™™™™{/} & This f0 turning point’s affiliation is ambiguous; it may be related to the Prosodic Structure event before the turning point, or to the Prosodic Structure event that follows it. \tabularnewline
\bottomrule
...
A second example, shown below, was also presented as Figure \ref{fig:really_would_not_annoy Points basic} during the discussion of Basic ‘™™™™{0}’ Point tier labels in Chapter \ref{ch:basics}.
...
In this case, we can see that there is a plateau stretching between the [m] and [l] of ®®®®{Molina}. The labeller has indicated an analysis on the Points tier that says the prominence on ®®®®{Sara} is a low-high rise (1-1-5, in the Levels labels) and ®®®®{Molina} as a high-mid fall (5-3, in the Levels labels). Notably, instead of the ‘™™™™{*</*>}’ single point in \texttt{sarah\_murray} as in Figure \ref{fig:sara_murray Points Adv}, in this example there are two points ‘™™™™{*<}’ and ‘™™™™{*>}’ at the beginning of ®®®®{Molina} (where there is ‘more space’ than in ®®®®{Murray}). This supports the analysis that the ‘™™™™{*</*>}’ in \texttt{sarah\_murray} is a kind of coalescence, as suggested in the discussion above.
...
We have already discussed the plateau at the beginning of this utterance, in Chapter \ref{ch:basics}. Given the labellers’ lack of phonological analysis for the plateau, these labels remain as ‘™™™™{0}’s, even when the label is otherwise using the wider array of Advanced Points labels.
...
\midrule \endhead
\rowcolor{green}™™™™{0} & No phonological analysis given by the labeller & N/A\tabularnewline
\midrule
™™™™{*>} & & …before the relevant ™™™™{*}\tabularnewline
™™™™{*<} & Prominence: ™™™™{*}, ™™™™{?*}, or ™™™™{**} & …after the relevant ™™™™{*}\tabularnewline
™™™™{*@} & & …with the relevant ™™™™{*}\tabularnewline
\midrule
™™™™{]>} & & …before the relevant ™™™™{]}\tabularnewline
™™™™{]<} & Phrase boundary: ™™™™{]}, ™™™™{?]}, ™™™™{]]} & …after the relevant ™™™™{]}\tabularnewline
™™™™{]@} & & …with the relevant ™™™™{]}\tabularnewline
\midrule
™™™™{[>} & & …before the relevant ™™™™{[}\tabularnewline
™™™™{[<} & Phrase boundary: ™™™™{[}, ™™™™{?[}, ™™™™{[[} & …after the relevant ™™™™{[}\tabularnewline
™™™™{[@} & & …with the relevant ™™™™{[}\tabularnewline
\bottomrule
\caption[An expanded set of labels for the Points tier, with several advanced options.]{An expanded set of labels for the Points tier, with several advanced options. The fundamental label ‘™™™™{0}’ is highlighted, with the remaining labels being for labellers with more experience. Note that, as always, a ‘™™™™{?}’ can be prepended to any of these labels to indicate labeller uncertainty about whether there is indeed a pitch turning point at this time.}
\end{longtable}


(\textit{Note: an individual ‘™™™™{]}’ on the PrStr tier is very often associated with multiple ‘™™™™{]>}’, ‘™™™™{]<}’, or ‘™™™™{]@}’ targets. Similarly, this sort of many-to-one relationship is also very common for prominence-type annotations.})
...
\centering
  \begin{tikzpicture}[node distance=4em, anchor=west, align=flush center, FLOW/.style={->, very thick}, LABEL/.style={draw, fill=yellow!80, inner sep=.75em}, scale = .625, transform shape]
    ...
    \node (node1down) [LABEL, below=1em of node1] {™™™™{?}};
    ...
    \node[LABEL, text width=8em, left=of node8] (node9) {™™™™{,}<\textit{value in Hz}>};
...
Another similar example with this “™™™™{:na}” label is provided with \texttt{minimum2}.
...
\caption{The labeller provides an educated guess of the pitch minimum during “®®®®{out there}”.%
...
{[\textit{min}]-[\textit{max}]} &
	...
	If the local pitch min is 244.2Hz and the local pitch maximum is 381.5Hz, the label should be ‘™™™™{240-385}’
	\tabularnewline
{[\textit{min}]([\textit{min2}]:na)} &
	...
	If the local attestable pitch min is 112, but the labeller intuits that the speaker has deliberately not reached a low ‘™™™™{110(90:na)-385}’
	\tabularnewline
{[\textit{max}]([\textit{max2}]:na)} &
	...
	If the local attestable pitch min is 192, but the labeller intuits that the speaker has deliberately not reached a high ‘™™™™{90-192(300:na)}’
	\tabularnewline
{™™™™{X-}[\textit{max}]} &
	...
	If the local pitch max is 264.8Hz and the local pitch maximum is not easy to determine, the label should be ‘™™™™{X-270}’
	\tabularnewline
{[\textit{min}]™™™™{-X}} &
	...
	If the local pitch min is 138.2Hz and the local pitch maximum is not easy to determine, the label should be ‘™™™™{135-X}’
	\tabularnewline
™™™™{NA} &
	...
	If the pitch is a long stretch of unreliable pitch tracking, the label for that unreliable stretch is ‘™™™™{NA}’
	\tabularnewline
\bottomrule 
...
\midrule \endhead
™™™™{\#X} & The PoLaR label to which this tag has been added is being analyzed as having property X (where X is a variable). \tabularnewline
\bottomrule
\caption[A proposal for a means to encode additional grammatical analyses.]{A proposal for a means to encode additional grammatical analyses. (Note that these \#X tags can be stacked, to create a PrStr label such as ™™™™{]\#X\#Y\#Z}, indicating a perceived boundary that has characteristics X, Y and Z.)}
\end{longtable}
...
These sorts of tags may be especially useful for languages that make use of more than two phrasal levels of prosodic hierarchy (e.g., a labeller of Korean may wish to use ™™™™{]\#AP}, ™™™™{]\#ip}, and ™™™™{]\#IP}; see \citealt{jun07}), or for languages\slash varieties\slash data whose prosodic hierarchy is less clear but for which more than two hierarchical elements can be identified. A labeller of such data where the phonological analysis is still uncertain may wish to use labels like ™™™™{]\#A}, ™™™™{]\#B}, ™™™™{]\#C}, etc., to remain agnostic about particular analyses.

Two examples are given below. The first is an example from MAE\_ToBI training materials (se also \citealt{beckman-05}). In this Advanced PoLaR annotation, each ™™™™{]} on the PrStr tier is annotated with MAE\_ToBI’s numerical labels for constituent categories in the prosodic hierarchy: ™™™™{\#2}, ™™™™{\#3}, and ™™™™{\#4}.
...
The second example comes from Korean.  In this example, each ™™™™{]} on the PrStr tier is marked with one of the boundary types in the analysis proposed in \citealt{jun07}, including ™™™™{]\#AP}, ™™™™{]\#ip}, and ™™™™{]\#IP}.
...
\centering
\includegraphics[width=.875\linewidth]{PrStr-asked_only_an_illusion-adv.png}
\caption{The labeller has added the ‘analysis tag’ ™™™™{\#CF}, to indicate that the prominence on “illusion” expones the grammatical category of contrastive focus.%
...
Contrastive focus is analyzed as having a grammatical relationship with prominence (and not with phrase boundaries) in English, as the ™™™™{*\#CF} label notates in Figure \ref{fig:only_an_illusion Beyond} above. On the other hand, contrastive focus in other languages is (also) grammatically related to phrase boundaries (e.g., contrastively focused constituents in Korean are marked with an additional phrase boundary; \citealt{jeonnolan17}); in such languages, the ™™™™{\#CF} tag should (also) be added to the appropriate ™™™™{]} or ™™™™{[} label.
...
\midrule
\endhead
™™™™{]\#X} & Phrase Boundary for a Phrase of Type “X” & A prosodic phrase ends at this point; the type of prosodic phrase is specified by “X” (e.g., ™™™™{]\#IP}) \tabularnewline\hdashline
™™™™{[\#X} & Phrase Boundary for a Phrase of Type “X” & A prosodic phrase begins at this point; the type of prosodic phrase is specified by “X” (e.g., ™™™™{[\#IP}) \tabularnewline\hdashline
™™™™{*\#X} & Strongest Prominence in a Domain & The constituent this occurs in is the most prominent in the prosodic domain specified by “X” (e.g., ™™™™{*\#IP}) \tabularnewline\hdashline
™™™™{*\#CF} & Contrastive Focus Prominence & The prominence marks Contrastive Focus meaning \tabularnewline
 \bottomrule 
...
\end{scope}
\end{tikzpicture}
\caption{The final low plateau and low plateau with an extra fall have conventionally been labelled as ™™™™{L-L\%} in MAE\_ToBI.%
...
\begin{itemize}
	\item At what rate do accents like ™™™™{L*} or ™™™™{H*} get realized with each of the possible Levels values?
	\item How do peaks\slash valleys (as labelled on the Points tier) align with Phone segments that are marked with a pitch accent label like ™™™™{H*} or ™™™™{L*}?
	\item How are the beginnings of phrase-final boundary plateaus (ToBI ™™™™{H-} and ™™™™{L-}) timed with respect to Phones segments and/or PrStr ™™™™{*} and ™™™™{]} labels?
	\item etc.
\end{itemize}
...
\begin{infobox}[frametitle=\textbf{Generating “Pseudo-Categorical” Labels from PoLaR Labels}]
	...
	Of course using PoLaR labels to construct the boundaries of a category requires an algorithm. A preliminary attempt at implementing such an algorithm is coded into a script in the PoLaR plugin for Praat. The basics of this algorithm are as follows. We begin with prominence-associated labels. The script identifies the Phones-tier segments during which a ™™™™{*} label occurs and any Points tier labels that are associated with that ™™™™{*} (through ™™™™{<}, ™™™™{>}, or ™™™™{@} labels in the Advanced Points labels). For every ™™™™{*}-associated Points label within the relevant Phones interval, the corresponding Levels label appears next to the ™™™™{*} in the Pseudo tier label. For ™™™™{*}-associated Points labels that precede the relevant Phones interval, the associated Levels label(s) is/are inserted, followed by a ™™™™{+} (akin to ToBI complex tones). For example, during “®®®®{librivox}” in \texttt{librivox\_recording-1}, there is a ™™™™{2+4*} label indicating that the Level 4 Point occurs during the ™™™™{*}-marked segment, and it is preceded by a Level 2 Point that occurs before the ™™™™{*}-marked segment. On the other hand, in \texttt{librivox\_recording-2}, a ™™™™{4+*+5} label occurs within “®®®®{librivox}”, indicating that neither Points label occurs within the ™™™™{*}-marked Phone segment, and that a Level 4 Point occurs before the segment, and a Level 5 Point follows it. 

	As for boundary-associated labels, the script identifies the Phones-tier segments after which a ™™™™{]} label occurs and any Points tier labels that are associated with that ™™™™{]}. For every ™™™™{]}-associated Points label within or after the relevant Phones interval, the corresponding Levels label appears before the ™™™™{\%} in the Pseudo tier label. For ™™™™{]}-associated Points labels that precede the relevant Phones interval, the associated Levels label(s) is/are inserted, followed by a ™™™™{-} (akin to ToBI phrase accents). For example, after “®®®®{recording}” in \texttt{librivox\_recording-1}, there is a ™™™™{1-3\%} label indicating that the Level 3 Point occurs during the ™™™™{]}-marked segment, and it is preceded by a Level 1 Point that occurs before the ™™™™{]}-marked segment. After “®®®®{librivox}” in \texttt{librivox\_recording-1} there is a ™™™™{1-\%} label, indicating that there is a ™™™™{]}-associated Level 1 Point before the phrase-final segment, and no ™™™™{]}-associated Point during the phrase-final segment.
...
\caption{MAE\_ToBI labellers may disagree or be uncertain about whether what’s most appropriate for the initial high pitch is a ™™™™{H*} label, a ™™™™{\%H} label, or no label at all. The different possible labels are indicated on the three “ToBI Alt” tiers.%
...
\caption{MAE\_ToBI labellers may disagree or be uncertain about whether a label of ™™™™{L+H*} or ™™™™{H*} is more appropriate for “®®®®{indeed}”. The different possible labels are indicated on the two “ToBI Alt” tiers.%
\label{fig:Usages and indeed}%
\index{Annotated example, Usage cases!and\_indeed}
}
\end{figure}

This disagreement or uncertainty in phonological labels may be because the labellers have different criteria for what constitutes an ™™™™{H*} vs. ™™™™{L+H*}, or it could be that they differ in their perception of which phonological category is expressed, or both. On the other hand, because Points and PrStr are labelled separately, the PoLaR labels can indicate which aspects of the signal were perceived differently by different labellers. For example, in Figure \ref{fig:Usages and indeed}, this PoLaR labeller analyzes the lower f0 at the beginning of the vowel as associated with the prominence (using the Advanced ™™™™{*>} label on the Points tier), but another labeller might leave this as a ™™™™{0} label indicating they are not analyzing it as associated with the prominence. This information will be useful in focusing discussions about which phonological label (if any) is most appropriate.
...
\caption{A steady rise to a peak in the final (unstressed) vowel of “®®®®{menu}”, which is not perceived as prominent or at a phrase boundary.%
...
\caption{A steady high through the final vowel of “®®®®{number}”, which is not perceived as prominent or at a phrase boundary.%
...
\midrule \endhead
™™™™{1} & within the lowest 20\% \tabularnewline
™™™™{2} & within the 20\%--40\% range \tabularnewline
™™™™{3} & within the 40\%--60\% range \tabularnewline
™™™™{4} & within the 60\%--80\% range \tabularnewline
™™™™{5} & within the highest 20\% \tabularnewline
\bottomrule 
