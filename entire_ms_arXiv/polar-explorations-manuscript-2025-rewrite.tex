...
\def\ipatext#1{{\fontspec[Mapping=tex-text, Ligatures=NoCommon]{Cambria}{#1}}}
\def\ipabrackets#1{[\ipatext{#1}]}
\def\ipaslashes#1{/\ipatext{#1}/}
%%%%%%%%%%%%%%%%%%%%%%%%
...
This monograph lays out an extensive set of guidelines that define the \textbf{PoLaR} framework for \textit{labelling the acoustic phonetic aspects of prosody}, based on \textbf{Po}ints, \textbf{L}evels, \textbf{a}nd \textbf{R}anges.  The aim of this system is to provide a means of annotating (both \uline{separately} and \uline{more explicitly}) information such as f0 scaling and alignment, that is already taken into account by labellers using other intonational annotation systems, but in less explicit ways.

Before later chapters lay out the details of PoLaR, Chapter \ref{ch:background} provides some background on prosody, prosodic annotation, and the motivation for developing this framework. Here a reader can find a sketch of the basic properties of PoLaR —describing the fundamentals of the system and how it fosters the explicit labelling of both phonetic and phonological aspects of the prosody of an utterance. These are transcribed on a number of different tiers, so that the relationships between these two types of labels can still be annotated. 
% TODO: add something here about transcription and the purposes of this handbook
Chapter \ref{ch:background} ends with an overview of some advantages of PoLaR labelling. The reader who is primarily interested in learning how to label with PoLaR can skip ahead to Chapter \ref{ch:basics} (for PoLaR Basic labels) and Chapter \ref{ch:advanced} (for PoLaR Advanced labels), which serve as the annotation guidelines, i.e. as a tutorial on how to label using PoLaR.\footnote{Note: The guidelines found in this monograph are for version 1.0 of PoLaR annotation (\citealt{ahn-21}). Any developments beyond version 1.0 will be available through the PoLaR repository: \href{https://doi.org/10.17605/OSF.IO/USBX5}{https://doi.org/10.17605/OSF.IO/USBX5}.}

After the official guidelines, Chapter \ref{ch:beyond} discusses ways in which PoLaR labelling is explicitly designed to be customized or expanded upon. This is followed by the final content chapter of this monograph, Chapter \ref{ch:advantages}, which contains a discussion of some usage cases for PoLaR, as well as its practical and theoretical advantages. Chapter \ref{ch:practical} concludes the monograph with a reference guide to PoLaR, which is followed by published references and appendices.
...
In addition to being formed of words, spoken utterances contain a wide range of other information about timing, intonation, prominence, phrasing, voice quality, rhythm, etc., often collectively called spoken prosody. (See \citealt{ladd08}, \citealt{beckmanvenditti11}, and \citealt{barnesshattuckhufnagel20} for some broad overviews.) These aspects of an utterance are sometimes called supra-segmental, because they can span regions larger than a single phonemic segment (i.e., a single consonant or vowel). (See \citealt{lehiste70} for extensive discussion.)

In a language like English, two major categories of prosodic structure concern \textbf{prominence} (related to notions of accent, stress, focus, emphasis, etc.) and \textbf{phrasing} (related to notions of grouping, disjuncture, pauses, etc.). In turn, both prominence and phrasing correlate with changes in \textbf{pitch} (related to notions of f0, tone, intonation, etc.). Speakers of English modulate these and other prosodic aspects of speech, such as duration, amplitude and voice quality, and thereby signal distinctive pragmatic, semantic, syntactic, or morphological information. In order to study these phenomena, linguists and speech scientists of many types are interested in annotating the prosodic structure of utterances.

As an example of the effect of prosody on linguistic structures and meanings that speech scientists and linguists have been interested in, consider the English string \langtextquot{Steve or Sam and Bob will come}. As discussed in \citealt{lehiste73} (also \citealt{price-91}, \citealt{veilleux-06}), manipulating the supra-segmentals that signal prominence and grouping in this sentence can change its meaning. In the following two pronunciations, capitalization indicates prosodic prominence and commas indicate prosodic phrasing.
...
\subsection{Notes on Annotation/Transcription}\label{sec:notes on annotation and transcription}
% TODO: add this!

\begingroup\color{red}
\begin{itemize}
	\tightlist
	\item add this after editing in google docs
	\item make changes throughout Ch 1 before this TO SIGNPOST THIS SECTION 
\end{itemize}
\endgroup

%\begingroup\color{red}
%\begin{itemize}
%\item 	what you transcribe should be tied to your particular GOAL
%\begin{itemize}
%\item so it is important to establish your goals when you start transcribing
%\end{itemize}
%\item ex1 let’s look at two different IPA transcriptions that are different but there isn’t one “correct” one (e.g., d vs flap; n vs nasalized flap)
%\end{itemize}
%\endgroup
...
(For further description of past prosodic models and annotation systems, please see, e.g., \citealt{roach94}, \citealt{ladd08} Chapters 1 and 2, \citealt{fery17} Chapter 5, \citealt{barnesshattuckhufnagel20}.) That said, \uline{\textbf{no} familiarity with these systems is required in order to learn and apply the basic aspects of PoLaR annotation}.
...
An example annotated according to the PoLaR labelling guidelines is given in Figure \ref{fig:PoLaR 1st basic}.\footnote{\uline{Interpreting the visualization of the audio (above the tiers of text):} The grayscale marks in the background correspond to the spectrogram, which shows the distribution of energy across frequencies. Here, darker gray means more energy, and the Hertz values of the frequencies can be read from the y-axis on the right side of the figure. The blue marks are the software-detected f0 measurements, and Hertz values can be read from y-axis on the left side of the figure.}
...
\section{The Contents and Purposes of this Handbook}
% TODO: add this!
%
...
This section lays out some of our intentions for the content of this handbook and for how it should be understood. After reviewing some fundamentals of what influences annotation tasks (\textit{which are not always recognized as such}), we clarify some issues of how to understand this this by enumerating some examples of what it does and doesn't provide. 

This section is written to achieve two purposes. First, to bring these fundamentals to the reader's conscious awareness, so that they can be more mindful of their labels. Second, this is to clarify how the handbook's design relates back to those fundamentals: aspects of these guidelines that may seem to be unintuitive (or to run counter to expectations based on other intonational annotation systems) often exist in service of relating to these fundamentals.

In particular, those who are familiar with other intonational annotation guidelines may need to re-set expectations on how to interpret particular examples or areas of guidance. Related to this last point, this section concludes with a review of the Ranges Tier which, perhaps more than other aspects of this annotation system, requires readers with past experience in intonational annotation to re-set expectations.

\subsection{Key Influences in Good Annotation}\label{sec:Key Influences in Good Annotation}

To begin, we ask readers to acknowledge the fact that a good PoLaR annotator will decide what to label, and how, based on multiple factors: (1) \textbf{the annotation framework} and its definitions, (2) the annotator's linguistic (conscious or subconscious) \textbf{perception} of language data, and (3) the particular (perhaps latent) \textbf{goals and questions} that motivate their work. Moreover, we cannot overstate the value of \textbf{human intuition} as a fourth key factor behind annotator choices in prosodic annotation, given that language data is inherently human and that intuition is essential for qualitative assessment of lesser-understood domains like prosody. 

Ultimately, as mentioned in the opening of this section, the contents and intended purposes of this handbook are rooted in these components of annotation. First, this handbook is written to provide clear definitions of key elements of the \uline{PoLaR framework}. Second, we include example annotations alongside a repository of recordings to help calibrate \uline{annotator perception} to PoLaR's definitions. Third, we try to connect differences in \uline{goals and questions} to different valid approaches to using PoLaR. Finally, throughout our guidance, we encourage annotators to have their labels match their \uline{intuition} and analysis, instead of aiming to match labels to prescribed norms.

\subsection{What this Handbook Does and Does Not Provide}

This handbook has been written to introduce the PoLaR annotation system, instruct readers on key aspects of the framework, describe the sorts of purposes it can serve in various contexts, provide examples of what can be annotated and how, and share WISDOM on some BEST PRACTICES. 

Since PoLaR is a novel system primarily designed as a tool for \textbf{phonetic transcription at a medium-to-fine granularity}, its intended usages and goals are distinct from many other intonational annotation systems (e.g., ToBI annotation systems) that are designed as tools for phonological analysis (or at least very broad phonetic transcription). Parallel to other phonetic transcription systems (such as the International Phonetic Alphabet), it is intended as a tool for annotators to record (relevant) \textbf{descriptive observations}. (See §\ref{sec:notes on annotation and transcription} for discussion of how different annotator goals can influence which types of observations are relevant and at what level of detail.) 

\subsubsection{Not Provided: Phonology-Driven Annotation Guidelines}

Phonology-based annotation systems (like ToBI) require a labeller to be well-versed in the relevant phonological grammar, and using that knowledge to decide how to (and even \textit{whether} to) label a given observed intonational modulation. There are many benefits to both types of systems: a ToBI-style annotation system allows for better inter-annotator agreement and facilitates theoretical analyses, while PoLaR offers more freedom to labellers and facilitates dealing with un-/under-described intonational systems.

This approach also allows PoLaR to capture a wider array of prosodically relevant features, including those not traditionally considered phonological, such as cues related to "emotional prosody" or other systematic, yet understudied, aspects of the acoustic signal. By keeping acoustic-phonetic cues distinct from abstract phonological categories, PoLaR aims to facilitate clearer analysis and reduce uncertainty for labellers. In this way, the purpose of PoLaR is not to \textit{be} a phonological analysis, but rather to provide rich, explicit annotated observations that can \textbf{\textit{inform} phonological analysis}. 

\subsubsection{Provided: Sample Annotations for Examples from Mainstream U.S. English}

[add here]

\subsubsection{Not Provided: Guidance on Achieving High Inter-Annotator Agreement}

While PoLaR can serve as a common language for discussion between multiple on their own perceived observations, high rates of inter-annotator agreement are \textit{not expected}. (Nor are they required or even necessarily desired; see below §\ref{sec:Expectations and Guidance for Variation in Annotations}.) This lack of expectation is driven by the same three factors identified at the beginning of section \ref{sec:Key Influences in Good Annotation}: (1) the small number of constraints on what must{\slash}mustn't be annotated at what level of detail (i.e., high degree of annotator freedom), (2) differences in annotator perception, and (3) variation in labellers' purposes for annotation. In other words, PoLaR is a flexible framework aimed at capturing an annotator's observations at a descriptive level that matches their intents.

This contrasts with guidelines for intonational annotation systems like ToBI. Those guidelines may include examples with multiple valid labels; but even those ambiguous examples often only have a small number of valid labels, and the majority of examples are presumed to only have one valid set of labels. This is possible for those systems as they are based on an asserted phonological grammar that underpins the annotation system (while PoLaR does not). The existence of such a grammar is what drives the goal of high inter-annotator agreement for such annotation systems; and without one, PoLaR is more like the IPA where there is no expectation of high inter-annotator agreement.

\subsubsection{Not Provided: Examples with a Single Correct Set of Labels}

More often than not, there are \textit{multiple} valid ways to PoLaR-annotate a given utterance, and so readers mustn't take any of the handbook's contents as guidance on how to identify “the” correct annotation. Moreover, wherever PoLaR labels are provided in this handbook, they should be read as \uline{one} \uline{plausible} set of observations describing the prosody of an utterance. Instead of defining what \textit{must} be labelled (and how), these guidelines are provided to define what \textit{can} be labelled (and how to interpret those labels).

To exemplify how these factors can lead to variability in valid annotations, let's consider some examples of annotation with the International Phonetic Alphabet (the IPA), which is similarly \textit{not} designed for high rates of inter-annotator agreement. As a first example, a single observed pronunciation of “printer” from Mainstream U.S. English could possibly be accurately transcribed \ipabrackets{ˈpɹ̥ɹɪ̝̃ːɾ̃ɚ} or as \ipabrackets{pɹɪnɹ̩}, in part because the IPA itself does not come with annotation guidelines on what must{\slash}mustn't be annotated. That said, the definitions provided by the IPA can make it (sufficiently) clear to a reader what exactly the annotator chose to include in each description. Moreover, with some background understanding of English phonology, a reader could understand that both are possibly valid transcriptions for the same pronunciation (even though there are more differences than similarities in annotation itself). Though both may be equally valid in principle, one of the two may be more favorable than the other for particular rhetorical or analytical purposes.

As a second example, a given pronunciation of “log” in Mainstream U.S. English (with some, but not much rounding in the low vowel) could be transcribed \ipabrackets{lɐg} or \ipabrackets{lɔ̜g}, in part depending on whether the annotator is sensitive to a cot-caught distinction in English. This underscores the role of perception and intuition in labelling, and that differences in labels could possibly reflect genuine differences in the labeller's cognitive representations of speech.

This is \textit{not} to say that just \textit{any} transcription is valid. A specific utterance of “tree” in Mainstream U.S. English could be validly transcribed a \ipabrackets{tɹi}, \ipabrackets{tʰɹi}, \ipabrackets{tʃɹi}, \ipabrackets{tɹ̥i}, \ipabrackets{ʈɹ̥iː}, or \ipabrackets{ˈtʰɹi} (among others), but it would certainly be invalid to transcribe that same utterance of “tree” as \ipabrackets{pʁi}. Such an annotation would run counter to at least one (if not more) of the key factors for deciding the appropriateness of a transcription (framework definitions, perception, and transcription goals). 

It is based on this logic that no example annotations provided in this handbook should be seen as the sole valid set of labels.

\subsubsection{Provided: Expectations and Guidance for Variation in Annotations}\label{sec:Expectations and Guidance for Variation in Annotations}

Instead of prescribing what must be annotated and how, this handbook aims to provide clear enough guidance on what different annotations signify and how a labeller can connect that information to their intuitions and observations. PoLaR can therfore serve as a common ground of understanding such that differences in labels reflect true differences in labellers' linguistic perception and/or what they described. As a result, researchers are able to avoid sacrificing valuable information for the sole purpose of achieving high inter-rater reliability.


For the reasons we just outlined, differences in annotations among labellers are to be expected; but not only that, they are often \textbf{valuable sources of information}. This perspective posits that variation is not always noise; instead, divergences in labelling can reveal important insights, prompting researchers to examine the signal more closely and refine their understanding. Therefore, the system prioritizes the capture of valuable information, even if it means not achieving 100\% identical labels. Interpreting PoLaR labels requires an open mind, acknowledging that a labeller's intuition or interspeaker variation in prosody can lead to legitimate differences in annotation.

In an ideal state, when labellers differ, it stems from genuine perceived differences of prosodic features, or an exploration of varying phonetic cues for the same phonological category, akin to divergent narrow IPA transcriptions. 

Our aim is to ensure that when labellers differ, it stems from a genuine \textbf{perception of different prosodic features} or an exploration of varying phonetic cues for the same phonological category, akin to divergent narrow IPA transcriptions. 


%This perspective posits that variation is not always noise; instead, divergences in labelling can reveal important insights, prompting researchers to examine the signal more closely and refine their understanding. Therefore, the system prioritizes the capture of valuable information, even if it means not achieving 100\% identical labels. Interpreting PoLaR labels requires an open mind, acknowledging that a labeller's intuition or interspeaker variation in prosody can lead to legitimate differences in annotation.

\subsection{The Ranges Tier as an Example of this Handbook's Goals and Contents}

A key design choice that embodies this philosophy is the \textbf{Ranges tier}, which permits labellers to define local pitch ranges independently of established phrase boundaries. We acknowledge that this approach may appear challenging from both theoretical and practical viewpoints, and is, in some respects, unprecedented in labelling systems. However, this design is motivated by empirical observations of phenomena such as downstep and upstep (as well as more speculative intuitions on nested tonal groupings). PoLaR does not seek to \textit{prove} a specific theoretical position regarding these phenomena, but rather to provide a tool to \textbf{assess the truth of claims} about the relationship between the acoustic signal and phonology. The Ranges tier enables the Levels tier to quantify \textbf{relative pitch height within a speaker's current intended range}, providing a medium-granularity description of the f0 contour that supplements direct f0 measures. While inferring these subjective high and low points can lead to inter-rater variability, PoLaR frames these inconsistencies as opportunities for discussion and further research into how pitch ranges are systematically modulated in speech. The expectation is not perfect agreement on exact range boundaries, but rather that labels will yield \textbf{similar-enough measures of f0 size{\slash}extrema and identical Levels labels} due to the broader quintile bands. This explicit annotation allows for a deeper understanding of phonetic implementation and supports exploratory analysis of diverse languages and speech varieties.

\hrule

A notable feature of PoLaR, and one that highlights this philosophy, is the Ranges tier, which allows labellers to define local pitch ranges independently of established phrase boundaries. This design choice, while acknowledged as potentially challenging from both theoretical and practical standpoints, is motivated by empirical observations of phenomena such as downstep and upstep (as well as more speculative intuitions on nested tonal groupings). The Ranges tier enables the Levels tier to quantify relative pitch height within a speaker's current intended range, supplementing direct f0 measures and providing a medium-granularity description of the f0 contour. We recognize that inferring these subjective high and low points can lead to inter-rater variability. However, rather than viewing these inconsistencies as problematic, PoLaR frames them as opportunities for discussion and further research into how pitch ranges are systematically modulated in speech. This explicit annotation allows for a deeper understanding of phonetic implementation and supports exploratory analysis of diverse languages and speech varieties.

\hrule


A salient feature of PoLaR, exemplifying this design philosophy, is the Ranges tier, which allows labellers to define local pitch ranges independently of established phrase boundaries. We acknowledge that this design choice is challenging from both theoretical and practical standpoints, and is in some respects unprecedented in labelling systems. However, it is strongly motivated by empirical observations of phenomena such as pitch-accent-related downstep and upstep in English across phrase boundaries, which current systems struggle to capture  (as well as more speculative intuitions on nested tonal groupings). PoLaR's aim is not to prove a particular theoretical position, but rather to provide a tool that can assess the truth of claims about the relationship between the acoustic signal and phonology. The Ranges tier enables the Levels tier to quantify relative pitch height within a speaker's current intended range, offering a medium-granularity description of the f0 contour that supplements direct f0 measures. While inferring these subjective high and low points can lead to inter-rater variability, PoLaR frames these inconsistencies as opportunities for discussion and further research into how pitch ranges are systematically modulated in speech. The expectation is not perfect agreement on exact range boundaries, but rather that labels will yield similar-enough measures of f0 size/extrema and identical Levels labels due to the broader quintile bands. This explicit annotation allows for a deeper understanding of phonetic implementation and supports exploratory analysis of diverse languages and speech varieties.




%For this reason, this handbook is designed:
%
%- …to educate on the overall annotation system  
%  - Its key design, its various purposes / use-cases  
%- …to instruct users on how to annotate with it  
%  - What can be annotated and how (NOT: what must be annotated)  
%  - Best practices
%
%- Note that this handbook primarily discusses how to annotate English speech with PoLaR  
%  - (especially mainstreamed U.S. English varieties)  
%  - But the system and definitions are meant to be flexible enough to apply to any language, dialect, register, or idiolect  
%- …to provide examples of possible annotations  
%  - But NEVER to prescribe “the” annotation (there is never just 1)
...
Consider the visualizations of the two recordings of \langtextquot{another greenish banana} in Fig. \ref{fig:greenish banana prominent greenish} and Fig. \ref{fig:greenish banana prominent another}.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-greenish_banana-basic.png}
\caption{\texttt{greenish\_banana}: There is one intonationally prominent word, “greenish”.
...
\caption{\texttt{greenish\_banana-another}: There is one intonationally prominent word, “another”.%
\label{fig:greenish banana prominent another}%
\index{Annotated example, PrStr tier (basic)!greenish\_banana-another}%
}
\end{figure}

Though the same words are used in both recordings (meaning the lexical stresses are in the same places), they differ in the location of the one intonational prominence. In the first, \langtextquot{greenish} is intonationally prominent (marked with a rise-fall pitch contour), while in the second it is \langtextquot{another}. A listener could hear this intonational difference, and it can also be seen in the visualization by which word shows a significant f0 rise-fall.\footnote{Note that the software-based f0 contour is neither entirely continuous nor entirely smooth, in contrast to how a listener might perceive the pitch contour. This is partially due to the (un)reliability of software-based f0-tracking algorithm and partially due to the nature of phonation. These issues are more fully discussed in §\ref{sec:intonational-contours-and-software-based-pitch-tracks}.}



\begin{infobox}[frametitle=\textbf{A NOTE ON PRAAT SETTINGS}]
% [reviewer comment]: For instructions on the Praat settings, consider adding details for the "Spectrum" display, and perhaps update that the plug-in now includes these default settings too (right?)
% TODO: for Byron to do. make sure this is updated for Praat's new settings
...
To annotate prosodic phrasing in English, we will adopt the convention from the AM tradition of only annotating the boundary at the end of a phrase. (The idea is that the end of one phrase coincides with the beginning of another.\footnote{This is not a given for all work on prosodic phonology. As such, in the Advanced labels chapter, we describe a means of labelling other boundary edges.}) We will use the \polarquot{]} symbol for this. The end of a phrase is typically aligned with the end of a word, and so when we use the \polarquot{]} annotation, we want to align it in the Praat TextGrid with respect to the word boundary of the phrase-final word. Refer to the third tier of the Figure \ref{fig:vegetable list phrases}, to see this. Because basic PoLaR labels only indicate where a phrase ends, in cases where there is a pause after a phrase (e.g., after \langtextquot{arugula} or \langtextquot{rutabaga} in Figure \ref{fig:vegetable list phrases}), the \polarquot{]} label is placed before the pause. (Again, for more examples of this PrStr labelling, more examples are given throughout this monograph and in the “examples” section below.)
...
\midrule \endhead
\polar{*} & Prominence & A syllable that has intonational prominence \tabularnewline
\polar{]} & Phrase’s Right Edge\footnotemark & The right edge of the final word of a phrase \tabularnewline
\bottomrule 
\caption{Two Basic labels for the Prosodic Structure tier (for English).%
\label{2 PrStr basic labels}%
}
\end{longtable}
\footnotetext{At this point, we are introducing a single phrase-edge label, marking the end of a phrase. In the advanced labelling section, we also introduce a label for marking the beginning of a phrase: \polarquot{[}. For purposes of basic labelling, we do not introduce this in part because of how often the end of one phrase is co-timed with the beginning of another (and also because of how ToBI is structured); that said, for other purposes (e.g., assessing phrase-initial phenomena), it would be advised that labellers make use of the \polarquot{[} annotation introduced later in this handbook.}
...
Similar to uncertainty labels for prominence, a labeller can indicate uncertainty about the presence of a phrase boundary using the \polarquot{?]} label. For example, in \texttt{librivox-zip1}, the labeller is uncertain of whether there is a phrase boundary between \langtextquot{Librivox} and \langtextquot{recording}:
...
Although changes in f0 are often very visually salient cues to prosodic structure, particularly prominences, PoLaR PrStr labels are situated with respect to the word string. Here the \polarquot{*} label is placed in the center of the transcribed \ipabrackets{ɪ} vowel rather than at the f0 peak. The \polarquot{]} label is at the end of the final word in the phrase. 
...
Similarly, the \polarquot{*} label is placed in the middle of the transcribed \ipabrackets{i} rather than in the high f0 that cues the prominence. 
...
% TODO: Maybe somewhere we could say 'Short examples excerpted from spontaneous speech can sometimes sound unlikely, due to extreme or unexpected cue patterns, even though in context these examples sound perfectly natural and expressive.'
% while writing this handbook, we have become aware of the degree to which contours heard out of context may be surprising (to intonational phonologists and naive speakers alike) and may even sound unnatural. what is true at a research level is that we NEED a way to annotate these things; they occur.
%in addition, at a didactic level, we have crated labspeech recordings to provide clearer illustrations of contours that we have observed (e.g., clarified by using text comprised solely of sonorants) — to some, these may sound especially non-natural out of context (even of they are faithful replicas, for the phenomenon at hand).
}
\end{figure}
...
\midrule \endhead
\polar{*} & Prominence & A syllable that has intonational prominence \tabularnewline
\polar{?*} & Possible Prominence & A syllable that might have intonational prominence \tabularnewline
\polar{]} & Phrase’s Right Edge & The right edge of the final word of a phrase \tabularnewline
\polar{?]} & Possible Phrase’s Right Edge & What might be the right edge of the final word of a phrase \tabularnewline
\bottomrule 
...
% TODO: add a paragraph around here that introduces the challenges in reliability of f0-tracking (and setting the settings in software) — and point the reader to section 2.2 (but leave 2.2 in place).
...
PoLaR allows two levels of annotation detail on the Points Tier: the default (\polarquot{0}) label is described in this chapter. (Advanced labels for associating the pitch turning points on the Points tier with the prosodic events on the PrStr tier, are described in Chapter \ref{ch:advanced}.)
...
The PoLaR plugin for Praat (found at the OSF repository linked from \href{https://www.polarlabels.com}{www.polarlabels.com}) automates the resynthesis – though only under the guidance of Points-tier labels provided by PoLaR annotators. (That is, the script is not fully automated, acting on its own; it relies on the human intuition of labellers.) With the plugin installed, users can click on the “Tier” menu and select “PoLaR: Resynthesize Straight Line Approximation”, and this will resynthesize the sound file, based on the f0 track and the timing of the Points tier labels.
...
One of the contexts where the pitch will often have noisy non-salient turning points is in the case of segmental effects on prosody (“microprosody”). While an experienced labeller may recognize microprosodic perturbations, especially in well known contexts (e.g., around a stop consonant or a sibilant such as \ipabrackets{s} or \ipabrackets{z}), this may be more difficult for a novice. We will illustrate this briefly here, but the reader is also referred to section \ref{sec:intonational-contours-and-software-based-pitch-tracks} for further discussion. This illustration here concerns the effects of \ipabrackets{s}: Compare the f0 tracking surrounding the \ipabrackets{s} on \langtextquot{nanasanana} (which was produced with flat pitch, to the point of sounding unnatural) to the f0 tracking of the resynthesized pitch (which sounds identical to the original, to a human ear):
...
The f0 track on the left is the original from Praat. The one on the right has been manipulated and resynthesized in Praat, such that the f0 is a flat line, with the same starting and ending f0 as the original. Listeners generally do not perceive the two audio files in Figure \ref{fig:original and resynth} as different because the f0 movements leading into\slash out of the fricative noise associated with the \ipabrackets{s} do not reflect changes in the speaker’s target intonational contour.  Instead, these movements reflect effects of the \ipabrackets{s} articulation on the rate of vibration of the vocal folds, and how the software tracks f0. In such cases, where it is clear that the f0 track is influenced by such microprosody, \textbf{the general guideline is to \textit{\uline{not}} put any labels on the Points tier} that would track apparent f0 falls\slash rises that are clearly due to microprosody. In this case, only an initial and final Point would be sufficient to mark the flat f0.
...
\caption{\texttt{minimum}, with the Points tier annotated with \polarquot{0} labels.%
\label{fig:minimum Points basic}%
\index{Annotated example, Points tier (basic)!minimum}%
}
\end{figure}

\uline{First}, note the first (leftmost) \polarquot{0} label on the Points tier. It indicates the point in time where (reliable) f0 tracking begins, which corresponds to the beginning of the rise that cues the prominence. (For labellers familiar with other systems like ToBI, take special note that the \polarquot{0} default label does not record any relationship between the pitch and the prominence.) Though Praat begins tracking pitch in the middle of \langtextquot{a}, there is no perceptual fall in pitch here, and so we do not want to add a Points label that would produce a falling line at the beginning of the recording. PoLaR labellers are advised to place Points tier labels at the beginning of (reliable) f0 tracking in this way.

\uline{The second \polarquot{0} label} is placed at the peak of the f0 track, the location where the pitch again changes direction from rising to falling. Note that the small perturbations between these first two points are not labelled. These are due to noise in the signal processing algorithms and display and are not meaningful in prosodic perception.

\uline{Finally:} The last two points mark the end of the steeper fall and the end of the pitch tract, respectively.

\uline{Overall}, note that the timing of the PrStr tier labels and the timing of the Points labels are completely independent of one another. In the PrStr tier, the \polarquot{*} is aligned to the middle of the stressed vowel, and the \polarquot{]} is aligned to the right edge of the phrase-final word. On the other hand, the Points tier labels are aligned according to the movements of the f0 contour, and not to any particular syllable, word, phrase, etc., and not to any object in the PrStr tier.

\paragraph{Basic Points Example 2:\label{basic-points-example-2}}

In the example in the figure below, the pitch events (rising slightly, falling in two steps) are time-aligned with the default \polarquot{0} label.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-me-basic.png}
\caption{\texttt{me}, with the Points tier annotated with \polarquot{0} labels.%
\label{fig:me Points basic}%
\index{Annotated example, Points tier (basic)!me}%
}
\end{figure}

Note the timing of the last two labels on the Points tier. The third \polarquot{0} occurs where the f0 curve changes trajectory radically, as expected. However, the final \polarquot{0} does \textit{\uline{not}} occur at the end of the recording or at the final phrase break (] in the PrStr Tier), because the f0 tracking stops before the recording does. In such a situation, one can simply place the \polarquot{0} label where the f0 track ends (or stops being reliable, in the sense of reliably tracking perception of the intonational contour).

\paragraph{Basic Points Example 3:\label{basic-points-example-3}}

Other types of pitch movements might present challenges where the pitch contour displays discontinuities and turns that are not perceptually salient, at least not with respect to the overall perception of the intonation. For example, consider the f0 track below.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-out_of_order-plateau-basic.png}
\caption{\texttt{out\_of\_order-plateau}, with the Points tier annotated with \polarquot{0} labels. Note that the f0 range displayed here is limited (50 Hz)in order to highlight the fluctuations, which will appear smaller in a typical display.%
\label{fig:out_of_order-plateau Points basic}%
\index{Annotated example, Points tier (basic)!out\_of\_order-plateau}%
}
\end{figure}

Labellers report hearing a steady high pitch at the beginning of the utterance, from \langtextquot{seems} through \langtextquot{it’s} despite the  visible pitch perturbations in the f0-tracking. Moreover, labellers do not perceive any prominence or juncture that would call for a \polarquot{*} or \polarquot{]} label. In order to capture this long, high plateau, points tier labels have to be placed only at the beginning and end  of this interval. This will also reproduce the plateau in a straight line approximation, such as the one created with the PoLaR Praat plugin’s “Resynthesize Straight Line Approximation” command (described in section \ref{sec:identifying-necessary-points-labels-with-straight-line-approximations}), shown in Figure \ref{fig:out_of_order-plateau Points basic resynth}.
...
Listeners hear the original recording as sounding intonationally the same as the resynthesized version, corroborating that the f0 tracking in the original recording is unreliable.
...
An example of this is below; attend to the fact that there are no Points tier labels in or near the word \langtextquot{not}, (even though it is perceived as prominent):

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-really_would_not_annoy-basic.png}
\caption{\texttt{really\_would\_not\_annoy}, with the Points tier annotated with \polarquot{0} labels.%
\label{fig:really_would_not_annoy Points basic}%
\index{Annotated example, Points tier (basic)!really\_would\_not\_annoy}%
}
\end{figure}

The small perturbations along the otherwise flat f0 track from \langtextquot{really} through \langtextquot{annoy} are microprosodic or noise related and are also not labelled. Listeners hear this interval as a sustained high tone and not as 2-3 separate peaks. Also note that the leftmost \polarquot{0} has been placed at the location of the first reliable f0. In this case, that label is not at the beginning of the f0 tracking; instead, its placement is meant to indicate that the labeller does not believe that f0 track produced by the software is accurate until a bit into the vowel of \langtextquot{rain}.


\subsubsection{Labeller Uncertainty}\label{sec:labeller-uncertainty-points}

As was the case when labelling prosodic events in the PrStr Tier, labellers might be uncertain about whether an f0 turning point exists. In many of these cases, using the Straight Line Approximation Tool or the Override Labels described in the next section can resolve these issues. However, if neither approach is helpful, the labeller can again employ the \polarquot{?} (as in \polarquot{?0}) to indicate that there might be a turning point at this location but it is not certain.
...
There are multiple reasons why an f0 track may be discontinuous or display faulty f0 points. First, f0 is the measurement of the vocal fold vibration rate (“voicing”) and there are naturally occurring speech events where the vocal folds don’t vibrate (e.g., voiceless segments such as \ipabrackets{f}) or they vibrate irregularly (“creaky” voice). While this produces gaps in regular vocal fold vibration, many of these are “corrected for” by the human perceiver, who perceives the pitch as continuous and steady.

In addition, the f0 tracking display itself (in software like Praat) might be inaccurate due to the software f0-tracking settings. Most speech applications require some calibration for each speaker, so that f0 displays are as accurate (and as easily read) as possible. For example, inappropriate settings for the maximum f0 value and/or the minimum f0 value under Praat’s “Pitch settings” can lead to pitch halving\slash doubling (in which the reported f0 value is half or double of the “true” value), while inappropriate settings for the voicing threshold and octave jump costs can lead to f0 dropout or spurious f0 readings.
...
Consider the following case, where the f0 track is unreliable during \langtextquot{out there} due to issues of phonation. In this case, there is creaky voice (or silence) for most of the interval, and creaky voice can seriously impede the f0 detection algorithm and lead it to incorrect f0 estimates.
...
Here, the labellers heard the pitch falling steadily from the stressed vowel of \langtextquot{raining} to the end of the utterance, and estimated that the pitch at the end point of the fall should be about 65Hz (arriving at this number using a methodology described below). Since the initial f0 contour is reliable in this example, the initial \polarquot{0} labels don’t need the help of the comma override. A resynthesized straight line approximation of this pitch, that perceptually matches the original, verifies there is no turning point near \langtextquot{out} even though the original software-generated f0 track (above) suggests there might be. This straight line approximation is shown in Figure \ref{fig:raining-creaky Points basic resynth}.
...
In a second case, the f0 tracking for the boundary-related tones is unreliable at best (in part because of non-modal phonation beginning in \langtextquot{order} and lasting through the end of \langtextquot{again}). Notice that though there are some valleys in \langtextquot{of} and the beginning of \langtextquot{order}, there are no perceptible falls in pitch there (as confirmed by the straight line approximations achieved through pitch resynthesis). The pitch flattens out during \langtextquot{again}, though there is a slight perceptible rise (this may have to do with the switch from creaky to breathy phonation); because the f0 tracking is unreliable, as is often the case at the ends of an utterance, the f0 values are hard-coded into the final two Points labels.
...
The f0 is unreliable in many locations. Using \polarquot{0} (as we do for utterances at the beginning and end, respectively, of reliable f0) would not capture valuable information about the falls. 
In this example, \polarquot{0,195} indicates “This turning point is associated with a phrase boundary object time-aligned after this point, on the Prosodic Structure tier. The pitch value for this point should be 195Hz.” Thus the \polarquot{,195} portion of the label overrides the f0-track (which lacks a pitch value at this point) and can be extracted as a replacement in later processing, such as comparing slopes of f0 movements.

This sort of “manual override” is not just for cases where f0 is absent but can be used for unreliable f0 as is the case with the final two turning points. The labeller judges that the last f0 interval is a mostly flat, rising from 185Hz to 190Hz, and uses \polarquot{0,185} and \polarquot{0,190} at each end to override whatever f0 value the f0 tracking software provides.

\paragraph{Pitch Override Points Example 3: Missing f0 values in voiceless speech\label{pitch-override-points-example-3}}

The case above is for a phrase end where f0 tracking is often unreliable, but this method of directly annotating pitch value approximations can be used for any Points tier object where the labeller is concerned that f0 tracking might not be reliable. Consider the example in Fig. \ref{fig:greenish banana Points comma}, which contains three Points tier objects with manual-override pitch values:
...
Let’s begin in the word \langtextquot{greenish}. Here the listener perceives a turning point in the middle of the voiceless \ipabrackets{ʃ}, and estimate its pitch value to be about 190Hz, given the slopes of the f0 track line segments that flank it. Additionally, the pitch is perceived to continuously fall during \langtextquot{banana}, so the final upward-inflected pitch at the end is perceived to be mis-tracked, and the labellers provide an inferred pitch value of 160Hz. Finally, the beginning of this utterance has an f0 at around 185Hz: the f0 tracking is a little jittery, so the labeller has appended the initial \polarquot{0} label with \polarquot{,185} so as to not have to worry about the software identifying an incorrect value for this turning point. Also note, again, the comma override is used to capture the final f0 in the utterance.
...
The placement of the Points label (with the comma override) in the middle of this voiceless interval creates a resynthesized straight-line approximation that sounds perceptually identical, helping to justify the time-alignment and comma-override value for the \polarquot{0,420} label.
...
\midrule \endhead
\polar{0} & A turning point in the f0 contour (where line segments meet in a straight-line approximation of the pitch) \tabularnewline
\polar{?0} & Labeller is uncertain whether it is necessary to annotate a turning point in the f0 contour, even after exploring options with the straight line approximation resynthesis tool \tabularnewline
\polar{0,X} & An f0 turning point whose pitch value is approximately X (a labeller’s approximation of the f0 when the software-based f0 tracking is unreliable) \tabularnewline
\bottomrule 
...
Viewing the entire utterance as a whole, one can perceive it as having three different \uline{local pitch ranges}, \textbf{each of which is labelled as an interval on the “Ranges” tier}. Consider the high and low pitch in \langtextquot{okay so this is old advice} compared to the highs and lows in \langtextquot{but is it necessarily bad advice}. Although the highs and lows in \langtextquot{okay\ldots advice} are much lower than in \langtextquot{but is it\ldots advice}, on their own, they can be perceived as \uline{locally} high (even though, e.g., \langtextquot{okay} is much lower than the later highs) and low. Similarly, these highs and lows are also different from the highs and lows in \langtextquot{that keeps on getting recycled through the ages}.
...
If the labeller has access to PrStr labels, they may find it useful to consider ranges with respect to phrases as delimited by the \polarquot{]} label. At the same time, domains for ranges need not be determined by phrases labelled in the PrStr tier. Even a short stretch of speech may well turn out to contain local pitch range changes, as described in the Advanced labels chapter (Ch.\ref{ch:advanced}). Conversely, sequential phrases may be perceived as part of the same range. Ranges may to some extent suggest groupings among phrases, such as those labelled with \polarquot{]} in the PrStr tier. In this way, PoLaR Ranges annotation gives the labeller the means to capture more complicated relations where phrase boundaries (as annotated via the PrStr tier) and ranges can be somewhat independent: ranges can be annotated delinked from other boundary cues.
...
At the same time, the labeller should be a little cautious, and give a little wiggle room from the measured f0 min\slash max (rounding up or down to a number ending in 5 or 0, typically by at least 1 or 2 Hz, and often by a few more), since precise numbers are influenced by a number of factors, including software settings. Thus, a measured f0 range of 112.4Hz to 273.2Hz should have the min rounded down and max rounded up, and then be annotated as an interval in the Ranges tier with a label like \polarquot{110-280}. In other words, \textbf{in the typical case, (rounded) measures of local f0 min and local f0 max can be used to determine the label for a Ranges interval}. (This is the default guidance; more complex cases will be discussed here and in section \ref{sec:ranges-advanced} of Ch.\ref{ch:advanced}.)

Figure \ref{fig:amelia-knew2 Ranges basic} shows an example of a file annotated with the Ranges tier. Here, the minimum f0 is 114.7 Hz (in the \ipabrackets{ʌ} of \langtextquot{minimum}), and the maximum f0 is 248.3 Hz in the \ipabrackets{l} of Amelia. The labeller has rounded down to a range minimum of 110 Hz and rounded up to a range maximum of 250 Hz.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Ranges-amelia_knew2-basic.png}
\caption{An example of a short utterance with single pitch range interval, and clear f0 tracking.%
...
\caption[An example of a short utterance with single pitch range interval.]{An example of a short utterance with single pitch range interval, and a region of unreliable f0 tracking at the bottom of the speaker’s range.%
\label{fig:greenish-banana Ranges basic}%
\index{Annotated example, Ranges tier (basic)!greenish-banana}%
}
\end{figure}

Unlike in the example in Fig. \ref{fig:amelia-knew2 Ranges basic}, which shows a smooth f0 track, this example in Fig. \ref{fig:greenish-banana Ranges basic} shows f0 tracking unreliability where the speaker has produced creaky voice at the end of the utterance. In this case, the lowest reliable f0 is in the \ipabrackets{æ} of banana, at 162 Hz. The labeller has chosen to round the bottom of the pitch range a bit lower, using 150 Hz. (The maximum of 345 Hz takes place during a region of reliable f0 tracking, and the labeller has rounded up to 350 Hz.) Errors in f0 tracking at the end of phrases are very common, especially when the pitch is low and/or the speaker ends up creaking. The labeller should use their best estimate of how low the perceived f0 goes. 
...
Conceptually, the Levels tier divides the pitch space within a given range (taken from the Range tier) into 5 equal bands or sub-ranges, as schematized in Figure \ref{fig:tomorrow_morning Levels basic}. In this example, the first point (falling during the \ipabrackets{m} of \langtextquot{tomorrow}) corresponds to a turning point whose value in the f0 track is located in the top band (shown in red). Accordingly, this point gets assigned a 5 in the levels tier. Likewise the second point, which while slightly higher in f0, is still within the red band representing level 5. The following point, later in the \ipabrackets{a} of \langtextquot{tomorrow}, corresponds to a slightly lower f0 that falls within the 4 band (shown in yellow), and therefore gets assigned a level 4. Points marked at both the end of \langtextquot{tomorrow} and the end of \langtextquot{morning} correspond to f0 points in the lowest band (in purple), and are assigned level 1. (Note that in this particular example, there is a point where the f0 tracker is unreliable during \langtextquot{tomorrow}, as the speaker’s voice has become creaky. Recall section \ref{sec:optional-f0-override-labels-for-annotating-pitch-points-without-a-reliable-f0-track}).
...
For example, the file \texttt{no\_cigar}, shown in Figure \ref{fig:no_cigar Levels basic}, has 2 ranges annotated: 1) from 125-175 Hz for the word \langtextquot{close}, and 2) from 110-145 Hz for the word sequence \langtextquot{but no cigar}. Note that the pitch-accent-related point in the \langtextquot{-gar} of \langtextquot{cigar} is at around 140 Hz, so corresponds to a High in the range for \langtextquot{but no cigar} (as reflected in the level 5 designation, visually represented as the highest colored band in the right side of the figure), but it is only a bit higher (in absolute f0) than the point in \langtextquot{close} that is at 130 Hz, so corresponds to a Low (as reflected by the level 1 designation, visually represented by the lowest colored band in the left side of the figure) in the range for that word.
...
% TODO: this is probably deletable here, though. maybe delete + leave a reference to where it is discussed.
% start delete
\begin{itemize}
...
First and foremost, a labeller should trust their auditory perception. If the labeller perceives that the pitch is locally low, but the f0 track looks locally high, this could be due to pitch doubling; for example, at the end of \langtextquot{rutabaga} in Fig. \ref{fig:to volunteer-halved f0-tracking}, the pitch is perceived to continually fall, but it shows as almost as high as the preceding peak. Similarly, if the f0 shows a low that sounds high to the labeller, this may be halving. In these cases, the f0 tracker commonly measures f0 values that are either half (e.g., the f0 tracker shows 70 Hz, where listeners perceive pitch corresponding to 140 Hz) or double (e.g., the f0 tracker shows 140 Hz, where listeners perceive pitch corresponding to 70 Hz) the “true” pitch for the speech.
...
% TODO: references need to be added from this gDoc: https://docs.google.com/document/d/1PKhH5R25GfpMx1LtoxfF316u60uLDnYpyy8YTngGxVs/

Another very common set of cases where adjusting software settings may not do enough to yield a clean f0 track has to do with the phonetic characteristics of certain speech sounds. (As noted earlier, these segmental effects are also known as “microprosodic” effects.) Algorithms that produce f0 tracks rely on temporally-regular and identifiable glottal pulse patterns in the speech signal, produced by vibration of the vocal folds; because speech sounds also vary with respect to this property, the f0 track is influenced by the segments themselves. Some of the core types of segmental effects on f0 tracks are listed here:

\begin{itemize}
\item Interruption of f0 by voiceless segments: Breaks in the f0 track occur during segments where there is at least some interval where there is no vocal fold vibration. These are the most straightforward interruptions of the f0 track.
\item Distortions of f0 in the vicinity of obstruents (stops and fricatives): There can be perturbations up\slash down (or other irregularities) in the f0 track in contexts like the transitions between a consonant and a neighboring segment, or during a voiced fricative. These obstruents cause particular changes in the air flow through the glottis and in air pressure in the mouth, which influence vocal fold vibration.
% TODO: maybe come back to this: should the following two bullet points be rearranged? or removed from this list?
\item Certain vowels' intrinsic f0 influences: Different vowels have different intrinsic f0 values, and so the f0 track is influenced by which particular phones occur during it. For example, \ipabrackets{i} has a higher intrinsic f0, and \ipabrackets{a} has a lower one.
\item[] (\uline{HINT}: Sonorant sounds produced with modal voicing yield fewer segmental effects on pitch, because they involve the most regular and most easily identifiable acoustic patterns with respect to f0.)
\end{itemize}

Consider the example in figure \ref{fig:marmalade cake f0-tracking}, which has the same intonational contour produced on utterances with very different segments. The first portion of the recording, \langtextquot{Mariana made the marmalade}, is mostly smooth and steady, because it is primarily composed of sonorant sounds (except the \ipabrackets{d} and \ipabrackets{ð}, where the f0 track is missing or noticeably less reliable). On the other hand, the second portion of the recording, \langtextquot{Jack baked the cake}, is much less reliable: during the stops and affricates in this utterance, no f0 was detected. Thus this is an example where the intonational contours are essentially the same, phonologically, while the f0 tracks are acoustically very different, due to segmental effects.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Appendix-marmalade_cake.png}
\caption{The same intonational melody produced on a sentence largely comprised of sonorant sounds (left) and on a sentence with many non-sonorants (right).%
\label{fig:marmalade cake f0-tracking}%
\index{Annotated example, f0 tracking!marmalade\_cake}%
}
\end{figure}

It is important for PoLaR labellers to understand where such segmental effects on f0 may occur, in order to distinguish between intonationally-relevant changes in f0 and segmentally-caused changes. The example in figure \ref{fig:design improvements f0-tracking} provides an illustration of an f0 track is heavily distorted by non-sonorant sounds in ways that are not of interest for PoLaR labelling.  In particular, note that the \ipabrackets{z} in \langtextquot{design} and the \ipabrackets{mp} in \langtextquot{improvements} both cause a significant drops in the f0 track, but they do not correspond to drops in pitch\footnote{Recall that we distinguish “f0” and “pitch”: f0 is encoded directly from the speech signal, while pitch is a psycho‑perceptual phenomenon. Return to §\ref{sec:terminology} for more on terminology.} — listening to this example, labellers perceive the pitch as rising steadily from the beginning of \langtextquot{design} to the peak in \langtextquot{improvements}, as reflected by the Points tier labels (which skip over both of these drops in f0).

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Appendix-design_improvements.png}
\caption{An example where non-sonorant sounds disturb the f0 tracking; this is especially noticeable in the \ipabrackets{z} of \langtextquot{design} and \ipabrackets{p} of \langtextquot{improvements}. As shown, PoLaR labels ignore such segmentally-caused modulations of the f0 track.%
\label{fig:design improvements f0-tracking}%
\index{Annotated example, f0 tracking!design\_improvements}%
}
\end{figure}

Consider now the example in figure \ref{fig:out of order-plateau f0-tracking}, which exhibits many small rises\slash falls in the f0 track. Note that the Points tier annotations reflect that the annotators perceive the pitch containing many fewer intonationally-significant turning points – for example, the Points annotation reflects a mostly steady pitch during \langtextquot{seems like it’s}. This perception of much smoother pitch is reflected in the straight line approximation resynthesis in figure \ref{fig:out of order-plateau resynth f0-tracking}.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Appendix-out_of_order-plateau}
\caption{An example with many perturbations in the f0 track, due to segmental effects.%
...
\caption{A resynthesis on the basis of the Points annotation, to reflect the much smoother intonational contour that the annotator perceives. This smoother resynthesis sounds indistinguishable from the original.%
\label{fig:out of order-plateau resynth f0-tracking}%
}
\end{figure}


\subsubsection{Voice quality effects}\label{sec:voice-quality-effects}
In addition to speech segments affecting how f0 is tracked, the voice quality of speech can also impact f0 tracking. Voice quality (a.k.a. phonation) refers to how the vocal folds (and the larynx more broadly) are configured during speech production. Modal voice (the “neutral” voice quality) does not impact f0 tracking, while other phonations such as creaky voice, breathy voice, and whisper voice can and do regularly impact f0 tracking. For example, during whispering or breathiness, the f0 track may be absent or values may appear somewhat scattered. 

A particularly frequent non-modal phonation is creaky voice; it is especially common at the ends of phrases and where the pitch is low. It is marked by irregular pitch periods and especially perceptual glottalization (which can be seen in a spectrogram as prominent vertical striations, as during the bulk of the final \ipabrackets{i} in Fig. \ref{fig:joey f0-tracking}). During creaky voice, the f0 may disappear or appear to jump suddenly upwards\slash downwards. Both of these effects can be seen in Fig. \ref{fig:joey f0-tracking}: there is creaky voice during \langtextquot{it to} and at the end of \langtextquot{Joey}; during \langtextquot{it to} there is no f0 tracking, and during \langtextquot{Joey}, the f0 tracking jumps suddenly upwards (at the beginning of \ipabrackets{i}) and then disappears until the speaker returns to modal voice at the end of the utterance.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Appendix-joey.png}
\caption{Example of creaky voice phonation disturbing f0 tracking.%
\label{fig:joey f0-tracking}%
\index{Annotated example, f0 tracking!joey}%
}
\end{figure}

Another example of the impact of creaky voice on f0 tracking can be seen in Fig. \ref{fig:no_cigar f0-tracking}. From the beginning of \langtextquot{but} through the \ipabrackets{n} of \langtextquot{no}, there is creaky voice during. During that time, there is no f0 tracking (even during the voiced \ipabrackets{ʌ} and \ipabrackets{n}), and when it returns, the f0 appears to be high and falling onto the \ipabrackets{o}; however, auditory perception indicates that the pitch is steadily rising during \langtextquot{but no} (as reflected in Points labels) because this f0 fall is due to the offset of creaky voice. (Notice also that there are segmental effects from the \ipabrackets{s} and \ipabrackets{g} of \langtextquot{cigar}.)
...
\subsection{Guiding principles for recognizing and dealing with f0 track disruptions}\label{sec:guiding-principles-for-recognizing-and-dealing-with-pitch-track-disruptions}

\begin{enumerate} \def\labelenumi{\arabic{enumi}.}
\item If the f0 appears to change locally in a way that is surprisingly rapid and not clearly part of a bigger pitch pattern, consider the context in which those bumps, spikes or jumps are occurring. Could they be due to segmental effects or pitch tracking errors? Check that the “pitch range” in Praat's the “pitch settings” is set appropriately.
\item For some f0 track disruptions, adjusting Praat’s “pitch” settings commonly does not result in visible f0 contours that match perceived pitch contours. This includes brief f0 bumps induced by nearby segments (as in Figure \ref{fig:out of order-plateau f0-tracking}), as well as voice quality effects (as in Figure \ref{fig:joey f0-tracking}). (\textit{Note for novices: Voice quality effects —especially from creaky voice— are especially common at the edges of prosodic groups.})
\item In PoLaR labelling, where possible, put Points labels at places in the recording where you believe that the measured f0 corresponds to the pitch you perceive at that region of the utterance
\item For places where you believe a point or range floor\slash ceiling occurs during unreliable pitch, annotate an estimate of what the f0 value should be at that point. (See detailed guidelines in the Advanced Points labelling section.)
\end{enumerate}
...
Note that we offer this \polarquot{[} label as a tool for those who would find using it to be helpful for their research purposes, and using it is by no means required. In other words, use the \polarquot{[} label if you are compelled to, but we take no stance on how often labellers should be using it (if at all). For some cases where \polarquot{[} labels may be useful for Advanced Points labels, see section \ref{sec:points-associated-with-phrase-initial-boundaries}.
...
\midrule \endhead
\rowcolor{green}
\polar{]} & Prosodic Phrase Boundary & A prosodic phrase ends at this point \tabularnewline
\polar{?]} & Possible Prosodic Phrase Boundary & A prosodic phrase may end at this point, but the labeller is uncertain \tabularnewline
\polar{]]} & Large Prosodic Phrase Boundary & A noticeably large prosodic phrase ends at this point \tabularnewline
\polar{[} & Prosodic Phrase Boundary & A prosodic phrase begins at this point \tabularnewline
\polar{?[} & Possible Prosodic Phrase Boundary & A prosodic phrase may begin at this point, but the labeller is uncertain \tabularnewline
\polar{[[} & Large Prosodic Phrase Boundary & A noticeably large prosodic phrase begins at this point \tabularnewline
\bottomrule
...
Just as there are levels of prosodic phrasing, in some cases different levels of prosodic prominence can be identified perceptually and/or acoustically. In such cases an advanced labeller may wish to distinguish some prominences from other prominences that are stronger. Thus in addition to the default \polarquot{*} label, PoLaR provides the \polarquot{**} label for prominences that are perceived as stronger than others.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-naomi_email-adv.png}
\caption{The labeller is annotating the perception that \langtextquot{Naomi} and \langtextquot{email} are both prominent, but that \langtextquot{Naomi} is significantly more prominent.%
...
As with all Advanced labels, there is no need to use \polarquot{**} in every utterance. The label \polarquot{*} can indicate a perceived prominence (\textit{of any type — and corresponds roughly to what intonational phonologists would call a ‘pitch accent’ in languages like English}); \polarquot{**} is provided to assuage the feeling of many labellers that they would like to capture the distinction between a typical degree of prominence and an extra-emphatic one. (As default guidance for when to use these, labellers should only use the \polarquot{**} label when it is very clear that one of the prominences is perceived as especially strong, as compared to the other prominent words in the context of the utterance.)

Additionally, as with the \polarquot{]} and \polarquot{]]} labels, PoLaR does not require the labeller to adhere to any particular rigorous definitions for different sorts of prominence across their entire corpus; a labeller may simply wish to distinguish different degrees of prominence in a given example (perhaps as a simple matter of perceptual strength, perhaps because of especially high f0 peaks, etc.). That said, labellers are encouraged to annotate in a “misc” tier what makes a \polarquot{**} prominence stand out as especially perceptually strong.

\subsubsection{Summary of Prominence Labels}\label{sec:summary-of-prominence-labels}

Basic labels are highlighted, to be used by all, with the remaining labels proposed for labellers who have more experience or who are targeting particular phenomena. As a reminder: prominence is cued by acoustics, but these labels are about the labeller’s \textit{perception} of prominence, in the given utterance context.

\begin{longtable}{clp{.5\linewidth}} \toprule \textbf{Label} & \textbf{Phonological Object} & \textbf{Significance of Label}\tabularnewline
\midrule \endhead
\rowcolor{green}
\polar{*} & Prominence & The constituent this occurs in is prominent\tabularnewline
\polar{?*} & Possible Prominence & The constituent this occurs in might be prominent, but the labeller is uncertain \tabularnewline
\polar{**} & Especially Strong Prominence & The constituent this occurs in is especially prominent \tabularnewline
\bottomrule
...
As disfluencies can influence prosody in measurable ways, a labeller may find it useful to label disfluent speech as such. For this purpose, PoLaR provides the \polarquot{d} label, which is to be marked on the PrStr tier, at the place in the word (or partial word, in the case of a cut‑off word) where a disfluency begins (\textit{roughly speaking}).%
% TODO: clean up this text; it was written in a hurry
\footnote{We present this annotation as an informal tool, useful for tracking labeller intuitions. To be clear, disfluencies take place over a period of time typically, and a complete tool for analyzing them would use more detailed annotation that helps track the additional information (including the timecourse). Annotators wishing to go beyond the guidance of PoLaR on labelling disfluencies (which we encourage in domains like this!) are referred to other resources on disfluency annotation (e.g., \cite{DUEL handbook, Hough, de Ruiter, Betz, & Schlangen, 2015; Brugos et al 2019}).} 
% TODO: update these citations (including: A CUE-BASED APPROACH TO PROSODIC DISFLUENCY ANNOTATION Alejna Brugos, Alison Langston, Stefanie Shattuck-Hufnagel, Nanette Veilleux, ICPhS 2019)

While disfluencies may differ prosodically from one another, this ‘d’ label can be used for all types. An example of a disfluency that corresponds to a momentary stutter (with no sense of disjuncture) is given in Figure \ref{fig:in_a_plane-disfluencies PrStr Adv}. Additionally, an example of a disfluency that induces a sense of disjuncture like that of a phrase boundary (due to the overlap in set of cues) is given in Figure \ref{fig:spoon_here PrStr Adv}. In both cases, the \polarquot{d} label is on the PrStr tier, time-aligned with the onset of the disfluent moment\slash region, or where the speaker cuts themselves off.
...
\begin{longtable}{cp{.3\linewidth}p{.45\linewidth}}
	...
	\midrule
	\endhead
	\polar{d} & Disfluency & A point in the utterance where there is a disfluency\tabularnewline
	\polar{\{d} & Start of a disfluent region & A point in the utterance where a disfluent region of speech begins\tabularnewline
	\polar{d\}} & End of a disfluent region & A point in the utterance where a disfluent region of speech ends\tabularnewline
	\bottomrule
...
In this section we have reviewed some ways to systematically label these more refined intuitions, beyond the simple contrast of presence\slash absence of \polarquot{*} and \polarquot{]} labels; e.g., using the contrast between \polarquot{*} and \polarquot{**} labels, and \polarquot{]} and \polarquot{]]} labels to differentiate degrees of prominence\slash phrasing. In addition to the examples provided earlier in this section, these more fine-grained contrasts are exemplified in Figure \ref{fig:out_of_order_huh PrStr Adv} and Figure \ref{fig:asked_only_an_illusion PrStr Adv}.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-out_of_order_huh-adv.png}
\caption[The labeller hears especially strong breaks after \langtextquot{huh} and after \langtextquot{again}.]{The labeller hears especially strong breaks after \langtextquot{huh} and after \langtextquot{again}; they also hear especially strong prominence on \langtextquot{order}.%
...
\caption[The labeller is at several points uncertain about prominence.]{The labeller is at several points uncertain about prominence (\langtextquot{she}, \langtextquot{believes}, \langtextquot{only}), while also hearing especially strong prominence on \langtextquot{illusion} and especially strong phrase boundaries after \langtextquot{still} and \langtextquot{illusion}.%
\label{fig:asked_only_an_illusion PrStr Adv}%
\index{Annotated example, PrStr tier (advanced)!asked\_only\_an\_illusion}
}
\end{figure}

The following example Figure \ref{fig:at_this_point-disfluencies PrStr Adv} includes instances of both labeller uncertainty about prominence (\polarquot{?*}) and a perceived disfluency (marked with the \polarquot{d} label). Note that some disfluencies have pauses immediately following and some do not; if it feels like the disfluency leads to new prosodic phrase boundaries, these boundaries should be annotated separately. (Likewise for disfluency-induced prominence.)

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-at_this_point-disfluencies-adv.png}
\caption[The labeller hears several disfluencies.]{The labeller hears several disfluencies, where the speaker restarts\slash recasts their speech; the \polarquot{d} labels that indicate this are at the cut-off point, before any existing disfluency-related pauses.%
...
\begin{longtable}{cp{.3\linewidth}p{.45\linewidth}}
	...
	\endhead
	\rowcolor{green}
	\polar{]} & Prosodic Phrase Boundary & The end of a Words interval where a prosodic phrase ends \tabularnewline
	\polar{?]} & Possible Prosodic Phrase Boundary & The end of a Words interval where a prosodic phrase may end (but the labeller is uncertain) \tabularnewline
	\polar{]]} & Large Prosodic Phrase Boundary & The end of a Words interval where a noticeably large prosodic phrase ends \tabularnewline
	\polar{[} & Prosodic Phrase Boundary & The beginning of a Words interval where a prosodic phrase begins \tabularnewline
	\polar{?[} & Possible Prosodic Phrase Boundary & The beginning of a Words interval where a prosodic phrase may begin (but the labeller is uncertain) \tabularnewline
	\polar{[[} & Large Prosodic Phrase Boundary & The beginning of a Words interval where a noticeably large prosodic phrase \tabularnewline
	\hline
	\rowcolor{green}
	\polar{*} & Prominence & The center of the Phones\slash Words interval that is prominent\tabularnewline
	\polar{?*} & Possible Prominence & The center of the Phones\slash Words interval that might be prominent (but the labeller is uncertain) \tabularnewline
	\polar{**} & Especially Strong Prominence & The center of the Phones\slash Words interval that is especially prominent \tabularnewline
	\hline
	\polar{d} & Disfluency & A point in the utterance where there is a disfluency\tabularnewline
	\polar{\{d} & Start of a disfluent region & A point in the utterance where a disfluent region of speech begins\tabularnewline
	\polar{d\}} & End of a disfluent region & A point in the utterance where a disfluent region of speech ends\tabularnewline
...
One valuable aspect of the PoLaR system is that it allows labellers to annotate aspects of the intonational phonetics (e.g., time alignment of turning points) separately from aspects of the intonational phonology (e.g., prominences or boundaries). This independence can be achieved through using \polarquot{0} labels on the Points tier, as we described for Basic Points labels in Chapter \ref{ch:basics}. These PoLaR Basic labels (consisting of \polarquot{0} labels and optional comma-overrides) can be revised in a second-pass of labelling: in the first pass, PoLaR Basic Prosodic Structure and Points tiers are labelled, and in the second pass, \polarquot{0} labels are replaced with more complex labels (described in this section) to capture relationships between the  labels on the Points tier with those on the PrStr tier. To understand these complex labels, we describe first what such labels are designed to capture, before going into the details of how to use them.
...
Below we provide a more comprehensive description of the Advanced PoLaR labels that can replace pitch turning points that are originally marked with a \polarquot{0} on the Points tier, to indicate that a given turning point is perceived to be prominence-marking or edge-marking (or both). A PoLaR Advanced Points label consists of a minimum of two components: (i) the \textit{\uline{type}} of phonological object the turning point is associated with, and (ii) a \textit{\uline{pointer}} to the relevant Prosodic Structure tier label’s time coordinate. We find that there are three different scenarios that arise when deciding whether a turning point is associated with a prosodic event labelled on the PrStr tier, and if so, with which one:

% TODO: figure out a way to present this information (in this list, and in the table) in a way that prevents glassy-eyes and prevents having different info on the same topic in two places (the list and the table)
% start todo
\begin{enumerate} \def\labelenumi{\arabic{enumi}.}
\item No PrStr labelled event is associated with the f0 turning point, or it’s not clear that one is. In this case, use \polarquot{0} as the complete label. (This is a ‘neutral’ label, indicating that the labeller is not committing to any particular analysis, but has determined that a significant turning point needs to be marked at that location.)
\item A particular \textit{\uline{type}} of PrStr labelled event (either \polarquot{*} or \polarquot{]}; marked below as \polarquot{\_}) is associated with the f0 turning point, which is time-aligned in various ways, indicated by one of three \textit{\uline{pointer}}s:

\begin{enumerate} \def\labelenumii{\alph{enumii}.}
	\item PrStr label follows Points label (\polarquot{\_>})
	\item PrStr label precedes Points label (\polarquot{\_<})
	\item {[\textit{uncommon}]} PrStr label is precisely co-timed with Points label (\polarquot{\_@})
\end{enumerate}
\item Occasionally, multiple PrStr labelled events are associated with a single f0 turning point. That is, the turning point is either (i) ambiguous between being associated with the preceding PrStr labelled event and being associated with the following event, or (ii) the labeller believes that the point relates to both of those events. In these cases a complex label is used, with two labels separated by a \polarquot{/} (\polarquot{\_</\_>}).
\end{enumerate}
% end todo

There are two observations to be made about these labels right away. First, any non-\polarquot{0} label on the Points tier indicates the labeller has an intuition about the appropriate analysis. Thus, in the absence of such an intuition, the advanced labeller should leave the \polarquot{0} label in place.

Second, the pointers themselves (\polarquot{>}, \polarquot{<}, \polarquot{@}) carry no significance beyond indicating a relationship of how a PrStr label and a Points label happen to be time-aligned. That is, because the timing of a PrStr label is at the discretion of the labeller, its precise location in time is not meaningful.  Thus, a label \polarquot{*>} in the Points tier simply means that “this turning point manifests a cue to the prominence labelled as a \polarquot{*} on the Prosodic Structure tier, which is time-aligned later than (to the right of) this Points tier label.” A minimally different label, \polarquot{*<}, says that the relevant PrStr \polarquot{*} label is time-aligned earlier than the Points tier label. To repeat, in the interest of clarity: these two labels are not in and of themselves informatively different, in part because the particular timing of the PrStr label is not informative.\footnote{In other words, the difference between, e.g., \polarquot{*>} and \polarquot{*<} is not a meaningful one; it may simply be a reflection of how the \polarquot{*} label happens to be timed – which is at the whim of the labeller. (Recall that the \polarquot{*} label on the PrStr tier need only be time-aligned within the stressed vowel.) This may be somewhat surprising for labellers familiar with other annotation systems, where labels that use different symbols in a label are meant to signify meaningful\slash phonological distinctions (e.g., \tobi{L+H*} vs. \tobi{L*+H} in ToBI or \tobi{H*L} vs. \tobi{!H*L} in IViE).}
...
\centering
  \begin{tikzpicture}[node distance=4em, anchor=west, align=flush center, FLOW/.style={->, very thick}, LABEL/.style={draw, fill=yellow!80, inner sep=.75em}, scale = .775, transform shape]
    ...
    \node[LABEL, below right=1em and 6em of node2.east] (node3no) {\polar{0}};
    \node[LABEL, above right=1em and 6em of node2.east] (node3yes) {\polar{*}\\\polar{]}\\\polar{[}};
    ...
    \node[LABEL, right=3em of node4.east] (node5) {\polar{>}\\\polar{<}\\\polar{@}};
    ...
    \node[LABEL, above=of node7yes.center] (node7) {\polar{/}};
...
In summary, these Advanced Points tier labels are either (1) the simple \polarquot{0} label, (2) a complex label with two components in the order \textit{<{type}>{}} \textit{<{pointer}>}, or (3) a label made up of two complex labels separated by a \polarquot{/}.
...
\polar{0} & The default label is still the most appropriate choice if the annotator chooses not to affiliate a turning point with a Prosodic Structure label. (No pointer is used in conjunction with a \polarquot{0} label.) \tabularnewline\hdashline
\polar{*} & An f0 turning point for a \polar{*} (\textit{of any type; e.g., also \polar{**} or \polar{?*}}) in the Prosodic Structure tier \tabularnewline\hdashline
\polar{]} & An f0 turning point for a \polar{]} (\textit{also \polar{?]} or \polar{]]}}) in the Prosodic Structure tier \tabularnewline\hdashline
\polar{[} & An f0 turning point for a \polar{[} (\textit{also \polar{?[} or \polar{[[}}) in the Prosodic Structure tier \tabularnewline
\midrule
\textbf{Pointer Label} & \textbf{Timing of the relevant Prosodic Structure Tier Object}\tabularnewline\hdashline
\polar{>{}} & This f0 turning point is related to a Prosodic Structure tier object that follows it \tabularnewline\hdashline
\polar{<{}} & This f0 turning point is related to a Prosodic Structure tier object that precedes it \tabularnewline\hdashline
\polar{@} & This f0 turning point is related to a Prosodic Structure tier object that is precisely aligned with it \tabularnewline
\midrule
\textbf{Special Label} & \textbf{Noted ambiguity} \tabularnewline\hdashline
\polar{/} & This f0 turning point’s affiliation is ambiguous; it may be related to the Prosodic Structure event before the turning point, or to the Prosodic Structure event that follows it. \tabularnewline
\bottomrule
...
This example also shows the usage of the \polarquot{]>} label. This indicates that the f0 movement labelled by the third and fourth points tier labels (formerly \polarquot{0}s) are related to the following phrase boundary, labelled with a \polarquot{]} on the Prosodic Structure tier. Note that there are \textit{\uline{two}} of these labels for this recording: one to mark the beginning of the low flat f0 that signals the end of the phrase (cf. ToBI phrase accents), and one to mark the end of that plateau. Also note that the final \polarquot{]>} label is not timed to be at (or even near to) the PrStr \polarquot{]}, but rather to label the end of the reliable f0 track.

A second example, shown below, was also presented as Figure \ref{fig:really_would_not_annoy Points basic} during the discussion of Basic \polarquot{0} Point tier labels in Chapter \ref{ch:basics}.
...
Again, note that the Points tier labels ignore unreliable f0 tracks (such as the very high f0 tracking at the beginning of \langtextquot{rain}, the dip in the \ipabrackets{d} of \langtextquot{would}, the perturbations between \langtextquot{not} and \langtextquot{annoy}, etc.). For further discussion of such phenomena, the reader is referred to Chapter \ref{ch:basics} (specifically section \ref{sec:intonational-contours-and-software-based-pitch-tracks}), for its discussion of f0 track disruptions (a.k.a. “microprosody”).

The first Point tier label, therefore, occurs where the initial f0 becomes reliable. Because that turning point is not perceived to contribute to any of the Prosodic Structure (PrStr) events, the \polarquot{0} label remains. The second and third Point tier labels are determined to be related to the prominence on the word \langtextquot{really} and so the default \polarquot{0} label is replaced with a \polarquot{*} to indicate its relationship to a prominence (rather than a phrase break) and the \polarquot{>} and \polarquot{<} pointers further indicate which prominence: the one labelled in the middle of the vowel in the first syllable of \langtextquot{really} in the Prosodic Structure Tier above.

Notably, \langtextquot{not}, though prominent, is not marked by any label on the Points tier; it occurs in the middle of a high plateau. To emphasize: not every prominence or phrase boundary in PrStr necessarily corresponds to a Points tier label. The fourth Points tier label is determined to be related to the prominence on \langtextquot{annoy}, marked at an earlier time point in the PrStr tier. Finally, the last two Points tier labels are associated with the final low phrase break to their right.

A broader point illustrated by this example is that there is no minimum or maximum number of Points tier objects that are related to a given PrStr tier object. Each of the three \polarquot{*} labels in this example is related to a different number of Points labels (in temporal order, the PrStr \polarquot{*}s are affiliated with two, zero, and one Points labels, respectively).
...
As an example, if a prominent syllable is phrase-final, a turning point could be associated with the \polarquot{*} or the \polarquot{]} on the PrStr tier, both of which are labelled on the same syllable. This is illustrated by the word \langtext{beans} in Figure \ref{fig:vegetable_list Points Adv} below.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-vegetable_list-adv.png}
\caption[The \ipabrackets{i} vowel in \langtext{beans} is prominent and is also phrase-final.]{The \ipabrackets{i} vowel in \langtext{beans} is prominent and is also phrase-final. The low f0 could be associated with the \polarquot{*} (i.e., a low pitch accent), or it could also be associated with the \polarquot{]} (i.e., as the beginning of a final rise edge tone).%
\label{fig:vegetable_list Points Adv}%
\index{Annotated example, Points tier (advanced)!vegetable\_list}
}
\end{figure}

Rather than require the labeller to arbitrarily assign the turning point label leftward or rightward, the \polarquot{/} can be used between two labels, allowing the labeller to indicate that they believe both PrStr associations are plausible. This is the case in Figure \ref{fig:vegetable_list Points Adv}, where \polarquot{*</]>} means the turning point could be associated with the \polarquot{*} on the PrStr tier that precedes the Point or with the \polarquot{]} on the PrStr tier that follows it. Listeners hear “the same” intonational contour being relevant for both \langtext{beans} and \langtext{arugula}; note the similarities between the two, in particular with respect to their Points labels. While there are two Points labels annotated in \langtext{beans} and three in \langtext{arugula}, the analysis in them is the same: there is a low point related to the prominence \polarquot{(*<)}, another low(ish) point related to the boundary (\polarquot{]>}), and a high point related to the boundary \polarquot{(]>)}. It just happens that in \langtext{beans}, two of these relationships are annotated at the same point in time (\polarquot{*</]>}). The additional syllables in \langtext{arugula} (particularly between the prominent \ipabrackets{ɹu} and the phrase-final boundary) provide enough time to see more clearly the motivation for each of the prominence and boundary being associated with a “low” point in the intonation.

Similar to this usage of \polarquot{*</]>}, the \polarquot{/} can be used to create compound labels like \polarquot{*</*>}, \polarquot{*>/]>}, \polarquot{]</*>}, etc. Additional examples with compound labels like this will be found in the following sections.

\subsection{Points associated with phrase-initial boundaries}\label{sec:points-associated-with-phrase-initial-boundaries}
While AM annotation conventions typically annotate only the boundary at the end of a phrase, PoLaR allows the flexibility of optionally marking the beginning of a phrase. In such cases, there may be f0 turning points that the labeller would like to attribute to an initial boundary. A widely-known circumstance in which this may occur is for a high initial f0 in a phrase that is not clearly associated with a prominence. (I.e. those rare cases that are labelled \tobi{\%H} in ToBI.) With PoLaR, the labeller may choose to assign any or all phrase-initial turning points to a \polarquot{[} boundary marker.

Figure \ref{fig:loan-percentH Points Adv} shows a case where an initial high f0 has been attributed to an initial High boundary tone (\tobi{\%H}) in ToBI training materials. In PoLaR, such a case could be labelled with a \polarquot{[} in the PrStr tier, and an early high f0 point in the Points tier with a \polarquot{[<} label:

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-loan-percentH-adv.png}
\caption[An initial high-f0 turning point is given a \polarquot{[<} label in the Points tier.]{An initial high-f0 turning point is given a \polarquot{[<} label in the Points tier, indicating affiliation with the \polarquot{[} object in the PrStr tier.%
\label{fig:loan-percentH Points Adv}%
\index{Annotated example, Points tier (advanced)!loan-percentH}
}
\end{figure}

As with other ambiguous cases as described above, there may be times when the signal is ambiguous, so that where there is a high f0 at the beginning of a phrase, a labeller may be uncertain whether there is a prominence or not. In such cases, the labeller can capture this ambiguity with a label such as \polarquot{[</*<} (accompanying, for example  a \polarquot{[} and \polarquot{?*} in the PrStr tier.), as is shown in Figure \ref{fig:honorariums Points Adv}.  This label indicates that the high initial f0 may be associated with either the left boundary, the prominence, or both.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-honorariums-adv.png}
\caption[An initial high-f0 turning point is given the label \polarquot{[</*<} label in the Points tier.]{An initial high-f0 turning point is given the label \polarquot{[</*<} label in the Points tier, indicating that the labeller considers the affiliation ambiguous between an initial phrase boundary and a potential prominence, labelled \polarquot{[} and \polarquot{?*} (respectively) in the PrStr tier.%
\label{fig:honorariums Points Adv}%
\index{Annotated example, Points tier (advanced)!honorariums}
}
\end{figure}

Researchers using PoLaR may opt to attribute any initial f0 points to phrase beginnings. The example \texttt{vegetable\_list} from Section \ref{sec:phrase-initial-boundaries} is repeated here in Figure \ref{fig:vegetable list Points left edge Adv} with Advanced Points tier labels indicating that initial f0 points are being attributed to the beginnings of each phrase.\footnote{Figure \ref{fig:vegetable list Points left edge Adv} shows examples of three or more consecutive Points with identical Levels values (e.g., \langtextquot{some beans}). It might seem, then, that this Points tier is “overlabelled.” However, as PoLaR labels can be used for strictly descriptive purposes (rather than analytic ones), there is nothing wrong, per se, with overlabelling the Points tier. In fact, novices might find it easier to overlabel than to underlabel, and should not be discouraged from doing so, as they learn. (When perception of Straight Line Approximations is used to inform location of Points labels, more labels will typically produce more faithful-sounding resynthesized f0 tracks.) Finding the threshold of what is just the right number of labels will improve through  practice and honing one’s “ear” for pitch perception. That said, the fact that Levels values are steady over three or more Points labels might be a signal to the labeller that their labels need adjusting., either by reducing the number of Points tier labels, or adjusting the Ranges tier (see §\ref{sec:how-many-range-intervals-different-approaches-and-their-implications} for discussion).}
...
First, examine this example of turning points in the f0 track each associated with distinct objects in the Prosodic Structure tier:
...
Again there is a single prominent word and the utterance is comprised of a single intonational phrase. In this case, the annotator has labelled the beginning of the reliable pitch with a \polarquot{0}, indicating they don’t believe this initial pitch is related to any particular PrStr object. Next, the rise in pitch (described with two f0 turning points: one at the base of the rise and one at the peak) is analyzed as being associated with one phonological object: the prominence labelled on the PrStr tier (at the midpoint of \ipabrackets{i} vowel in Am\textbf{\uline{e}}lia, as transcribed in the Phones tier). We annotate our analysis (that these two turning points are associated with this single phonological object) by using the \polarquot{*>} and \polarquot{*<} labels for two Points tier objects. Next, the pitch falls to a steady low, which has been analyzed as related to the phrase boundary labelled on the PrStr tier. The beginning of this steady low is annotated with a \polarquot{]>} label. The end of this steady low has been annotated with a \polarquot{]@} label: the \polarquot{]} label on the PrStr tier and the f0 turning point on the Points tier are positioned at the same point in time, and so a @ pointer is used (instead of the more common \polarquot{>} or \polarquot{<}).
...
As in the previous examples, most turning points can be analyzed as being associated with PrStr events. Turning points 1 and 2 are associated with the low pitch accent on \langtext{Sara} and the last two turning points are associated with the phrase boundary. However, the label on the 3rd point, \polarquot{*</*>}, indicates that this turning point is possibly associated with the preceding pitch accent (\polarquot{*<}; i.e., \langtext{Sara} is associated with a late-rising pitch accent), with the following pitch accent (\polarquot{*>}; i.e., \langtext{Murray} is associated with an early-falling pitch accent), or possibly even both (as a kind of coalescence). Finally, the 4th and 5th turning points, both labelled \polarquot{]>}, indicate that the labeller hears the word \langtext{did} as flat, with this plateau as cues to the phrase-final boundary. Before continuing on, one might note that there appears to be a visible shift in the slope of the fall curing the \ipabrackets{ɹ} of \langtext{Murray}. To see if this should be labelled as a turning point in the intonational contour, one can use straight-line approximation methods to attempt to resolve the uncertainty (e.g., pitch manipulation, as discussed in section \ref{sec:intonational-contours-and-software-based-pitch-tracks} of Chapter \ref{ch:basics}. However, the results might not be definitive – in such a case, a labeller can use the \polarquot{?0} label where they believe there may be a pitch turning point. (A labeller might also choose to postpone using the algorithm until later, and until this point is using \polarquot{?0} as a kind of placeholder.)

\paragraph{Advanced Points Example 4: A minimal pair with above}
In the previous example, \texttt{sara\_murray}, there is a turning point that could be interpreted as cuing the \polarquot{*} on \langtext{Sara} or the \polarquot{*} on \langtext{Murray}. Compare that with what might be perceived as “the same” intonational contour, overlaid on a slightly longer utterance – one with an additional unstressed between the stressed syllables of \langtext{Sara} and \langtext{Murray}:
...
In this case, we can see that there is a plateau stretching between the \ipabrackets{m} and \ipabrackets{l} of \langtext{Molina}. The labeller has indicated an analysis on the Points tier that says the prominence on \langtext{Sara} is a low-high rise (1-1-5, in the Levels labels) and \langtext{Molina} as a high-mid fall (5-3, in the Levels labels). Notably, instead of the \polarquot{*</*>} single point in \texttt{sarah\_murray} as in Figure \ref{fig:sara_murray Points Adv}, in this example there are two points \polarquot{*<} and \polarquot{*>} at the beginning of \langtext{Molina} (where there is “more space” than in \langtext{Murray}). This supports the analysis that the \polarquot{*</*>} in \texttt{sarah\_murray} is a kind of coalescence, as suggested in the discussion above.

\paragraph{Advanced Points Example 5: Non-PrStr-related Points labels}
There is always a possibility that a labeller might want to analyze a movement (here, a rise) in pitch that is neither prominence-related nor boundary-related. Such an example is below:\footnote{Note that the fourth Points tier label is \polarquot{*@} and not \polarquot{**@}. The \polarquot{*@}, \polarquot{*<}, and \polarquot{*>} labels are meant to point at any type of \polarquot{*} label (i.e., \polarquot{*}, \polarquot{**}, \polarquot{?*}) on the Prosodic Structure tier. Similar for \polarquot{]>} on the Points tier pointing to a \polarquot{]]} on the PrStr tier.}

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Points-menu_eleven-adv.png}
\caption[\texttt{menu\_eleven}, with Advanced PoLaR labels.]{\texttt{menu\_eleven}, with Advanced PoLaR labels. Note the placement of the comma-override label at the end of \langtextquot{eleven}: it comes after the \polarquot{]>}. %
\label{fig:menu_eleven Points Adv}%
\index{Annotated example, Points tier (advanced)!menu\_eleven}
}
\end{figure}

Since the initial f0 movements at the very beginning of the utterance are not always reliable, a \polarquot{0} here marks the first reliable f0. However, there is a perceptible rise, beginning with the low f0 (in \langtext{menu}). While the low f0 itself is here analyzed as contributing to a sense of prominence on \langtext{menu}, this labeller has used a \polarquot{0} label at the peak, to \textit{\uline{avoid}} analyzing the rise to the high in the unstressed syllable of \langtext{number} as a cue to a PrStr phonological object. In this way, while many AM framework annotation systems do not provide labels for an f0 movement unrelated to prominence\slash boundary, PoLaR allows the labeller to indicate a phonological analysis of pitch turning points, but the labeller is \textit{\uline{not required}} to do so. Moreover, even when committing to a phonological analysis, these annotation conventions allow a labeller to systematically label f0 movements that are not predicted by the current state of their chosen phonological theory.

It is possible that a different labeller with a different set of theoretical tools and analyses might indeed want to label the turning point in \langtext{number} as related to some phonological object. PoLaR embraces this variety in approaches, and does not require that its labellers adhere to any particular theory. Instead, \textbf{what PoLaR aims to do is allow the labeller to \textit{\uline{explicitly}} annotate aspects of intonational phonetics} (e.g., in the Points tier), \textbf{intonational phonology} (e.g., in the Prosodic Structure tier), \textbf{and mapping relationships between the two} (e.g., with non-\polarquot{0} labels on Points tier).

These non-\polarquot{0} labels should be used whenever the labeller is confident in a particular phonological analysis. On the other hand, if the labeller cannot decide on (or does not feel comfortable with) a (complete) phonological analysis, they can simply use the \polarquot{0} label for either or both of the two f0 turning points. \textbf{As a general rule, the labeller is advised that they can always use a \polarquot{0} label.} To reiterate what was said earlier, the default should be seen as: every perceptually salient turning point labelled in the Points tier can be labelled with a \polarquot{0}. Advanced labels are provided when the labeller is comfortable making assertions about the relationship between changes in f0 and intonational phonology.

\paragraph{Advanced Points Example 6: No commitment to an analysis}
Another example of using the \polarquot{0} label for lack of phonological commitment is given below.\footnote{A possible MAE\_ToBI label for this would have an \tobi{H*} on \langtext{seems} and an \tobi{H+!H*} on \langtext{out}. Such labelling would indicate either (i) \langtext{seems} is prominent, or (ii) \tobi{H*}s occur on non-prominent syllables. Indication (i) would not comport with the labellers’ intuitions here, that \langtext{seems} is not prominent (see Prosodic Structure tier). While indication (ii) would require non-canonical views on MAE\_ToBI, some labellers may adopt it; but adopting such a view would make \tobi{H*} a label for prominence and high pitch, or no prominence and high pitch. PoLaR gets around these complications by giving the labeller the freedom to explicitly label prominence separately from the perceived pitch.}
...
We have already discussed the plateau at the beginning of this utterance, in Chapter \ref{ch:basics}. Given the labellers’ lack of phonological analysis for the plateau, these labels remain as \polarquot{0}s, even when the label is otherwise using the wider array of Advanced Points labels.

This example also has two notable movements in the f0-tracking where the labeller is uncertain. The first is the dip in the tracking between \langtextquot{out} and \langtextquot{order}, and the other is the rise in the tracking at the end of \langtextquot{again}. In both cases, the labeller has begun their Points tier labels with a \polarquot{?} to indicate their uncertainty about whether there is a turning point. (One factor contributing to the uncertainty is likely that this utterance is produced in a very narrow pitch range: 100-150Hz, making it hard to ascertain whether a small f0 movement is significant enough to merit a Points label or whether it may be a microprosodic effect, as in the case of the dip during \langtextquot{of} that might be related to the \ipabrackets{v}.)
...
The table below lists 7 Points Tier objects that a PoLaR labeller can use. It is possible to add a \polarquot{?} before each label, to indicate labeller uncertainty about the presence of a pitch turning point, and it is also possible to include a numeric value after each label, to indicate labeller intuition about the pitch value (in Hz).
...
\midrule \endhead
\rowcolor{green}\polar{0} & No phonological analysis given by the labeller & N/A\tabularnewline
\midrule
\polar{*>} & & …before the relevant \polar{*}\tabularnewline
\polar{*<} & Prominence: \polar{*}, \polar{?*}, or \polar{**} & …after the relevant \polar{*}\tabularnewline
\polar{*@} & & …with the relevant \polar{*}\tabularnewline
\midrule
\polar{]>} & & …before the relevant \polar{]}\tabularnewline
\polar{]<} & Phrase boundary: \polar{]}, \polar{?]}, \polar{]]} & …after the relevant \polar{]}\tabularnewline
\polar{]@} & & …with the relevant \polar{]}\tabularnewline
\midrule
\polar{[>} & & …before the relevant \polar{[}\tabularnewline
\polar{[<} & Phrase boundary: \polar{[}, \polar{?[}, \polar{[[} & …after the relevant \polar{[}\tabularnewline
\polar{[@} & & …with the relevant \polar{[}\tabularnewline
\bottomrule
\caption[An expanded set of labels for the Points tier, with several advanced options.]{An expanded set of labels for the Points tier, with several advanced options. The fundamental label \polarquot{0} is highlighted, with the remaining labels being for labellers with more experience. Note that, as always, a \polarquot{?} can be prepended to any of these labels to indicate labeller uncertainty about whether there is indeed a pitch turning point at this time.}
\label{tab:adv points associating with prstr}
\end{longtable}


Note that it is possible —and indeed common— to find that any given \polarquot{]} or \polarquot{*} event annotated on the PrStr tier has more than one label on the Points tier associated with it.  For example, the utterance-final \polarquot{]} in Figure \ref{fig:out_of_order-plateau Points Adv} has three Points objects labelled with \polarquot{]>}. In other words, a single phonological element can be associated with {multiple} acoustic events in the f0 contour. (Phrasing it this way may indeed make this point seem trivial, but it is a departure from other intonational annotation systems where labels are only ever phonological.)

In other words, there is \textit{no expectation} for a one-to-one relationship between advanced Points tier labels (of the sort described in Table \ref{tab:adv points associating with prstr}) and associated PrStr tier labels. Indeed, it is \textit{common} for PoLaR annotations to exhibit a {many-to-one} relationship between a given Points tier label and its associated PrStr tier label; and this is true for annotations of both phrasing and prominence.

Additionally, there can even be one-to-many ({and many-to-many}) relationships associating Points labels and PrStr labels. An example of this includes the \polarquot{*</*>} Points label in Figure \ref{fig:sara_murray Points Adv}, where a single Points label is associated with multiple entirely different PrStr tier labels. This is reflective of an analysis where the labeller sees different phonological elements having a single acoustic event. (There could be various contexts that would give rise to this, including where multiple phonological elements each give rise to the same kind of acoustic outcome, which happens to manifest in the same point in the speech signal. Alternatively, this could arise from a process by which two phonological elements coalesce into a single acoustic outcome.)
...
\centering
  \begin{tikzpicture}[node distance=4em, anchor=west, align=flush center, FLOW/.style={->, very thick}, LABEL/.style={draw, fill=yellow!80, inner sep=.75em}, scale = .625, transform shape]
    ...
    \node (node1down) [LABEL, below=1em of node1] {\polar{?}};
    ...
    \node[LABEL, text width=8em, left=of node8] (node9) {\polar{,}<\textit{value in Hz}>};
...
\end{tikzpicture}
\caption[An idealization of pitch ranges in four hypothetical utterances.]{The y-axis is f0 (in Hz) and each of the bars indicates the speaker’s local pitch range in particular (hypothetical) utterances; successive utterances are indicated by color changes. (This idealized representation of pitch ranges corresponds to the perceived pitch patterns in the recording shown in Figure \ref{fig:long-story-ranges Ranges Adv}.)%
...
\end{figure}
% The Reviewer: The audio example for Figure 2.14 ("really_would_not_annoy"), Figure 3.29 ("long-storyranges") and Figure 3.50 ("totally_right-high") do not sound like natural speech.
% TODO: we created some prose about the "naturalness"/"realness" of these recordings and how they should be thought of. find it and refer to that here.
% possibly:  "for the purposes of demonstrating the labelling system of PoLaR and what sorts of phenomena it can be used to annotate, this manuscript includes visualizations (and where possible, the repository also includes .wav and .Praat files) of annotated speech with various types of examples: spontaneous, experimental, contrived, hypothetical, etc. The goal for these example annotations is not to make any generalizations or to assert any claims about English (or any language) and its linguistic systems, per se. Instead, the goal is to demonstrate what aspects of intonation a PoLaR annotator can track."

Consider the interval following the first pause (\langtextquot{a 9 year\ldots}), labelled \polarquot{100-225}, and compare that to the following interval (\langtextquot{made a\ldots}) labelled \polarquot{250-340}. The specific annotation indicates that the labeller perceives that a pitch of 225 Hz on \langtext{Maryland} would certainly count as a “high” (and would be transcribed with a value of \polarquot{5} on the Levels tier) in the first part of this utterance. However, moments later (during \langtextquot{made a…}), 250Hz is perceived as the “low” floor of this interval (and would be transcribed with a value of \polarquot{1} on the Levels tier).

In particular, the relevant aspect of intuition concerns “what goes together” as a pitch range constituent. To make this clear, let’s zoom in on \langtextquot{he did it by selling I think Google stocks} in the previous example and discuss why the labeller has labelled \langtext{I think} as a pitch range (sub)interval, rather than including \langtext{I} as part of the preceding pitch range interval, despite the fact that it begins with a low f0 very close to the f0 of the preceding word.
...
Returning to the example, let us review three reasons why one might want to label the Range interval constituent as the word sequence \langtext{I think} (and, e.g., not just the single word \langtext{think}). Let’s start with an f0-based intuition about constituency. When the annotators labelled this, they intuited that there is a single pitch movement —a large rise from low to high— which includes the pitch values during \langtext{I}. That is, the relevant pitch movement does not occur solely within the word \langtext{think} (where there is a rise from around 325Hz to 410Hz); instead, it is intuited as a rise from within \langtext{I} to within \langtext{think} (a rise of about 105Hz to 410Hz). It may be that if this interval consisted of continuous sonorant material, e.g. \langtextquot{I wink}, we would see more clear pitch-based evidence that there is a single f0 ‘scoop’ that starts in \langtext{I}. A second way in which prosody may contribute to the intuition of constituency might be based in the fact that there is a brief period of glottal closure between \langtext{selling} and \langtext{I}, resulting in a brief silence and initial strengthening in the form of a glottal release at the onset of \langtext{I}. Since glottal marking of this kind is common in American English (see \citealt{dilley-96}), its presence here may contribute to the intuition that a new constituent begins at the onset of \langtext{I}. Third and finally, \langtextquot{I think} maps on to a syntactic\slash semantic constituent (the evidence for which we won’t discuss here). If we kept the f0 the same, but changed the words from \langtextquot{…selling I think…} to \langtextquot{…selling it but…} (where the extra high pitch falls entirely on \langtext{but}), there might be some cause to shift the Ranges interval to encapsulate only \langtext{but} – \langtextquot{it but} is not a syntactic\slash semantic constituent. (Of course, one would also need to weigh these factors against other cues, such as the placement of glottal events.)
...
In other words, a single Ranges interval here does not capture the intuition of a significant pitch rise during \langtextquot{or-}, since that Ranges interval places both Levels labels in the same pitch quintile.

By contrast, to account for listeners who hear a rise in pitch during order, the Ranges tiers can be labelled as in Figure \ref{fig:out_of_order-ranges 2range no() Ranges Adv}. The beginning of the utterance is given a range of 150-400Hz, but the second range has a compressed range of 150-230Hz, encoding a local rise in pitch.\footnote{Note that the first Ranges interval has a pitch floor of 150Hz, which is not attested in this portion of the utterance. We clarify why one might want this to be 150Hz in discussions of pitch floors (section \ref{sec:ranges-and-pitch-floors}) and Levels (section \ref{sec:the-influence-of-levels-and-ranges-on-one-another}).}
...
These labels could be generated stepwise as follows. First, a labeller adding Basic Ranges labels could label the entire recording as one Ranges interval of \polarquot{150-400}; then, when an advanced labeller is adding more fine-grained labels, they notice the pitch range compression after \langtextquot{seems like} and so they divide the range in two, and in the second range they add a new, lower pitch ceiling label in parentheses: \polarquot{150-400(230)}. In other words, here the value in parentheses indicates \emph{the change} from the Basic label. Thus the advanced labeller has captured their intuition that the pitch range is compressed, in direct relation to the range labelled \polarquot{150-400} (i.e., that this is some kind of range compression operation that subordinates the latter range to the former one).
...
\end{tikzpicture}
\caption{For expository purposes, consider this hypothetical f0 track for an utterance. In principle, this could be a realization by a speaker who is selecting a local pitch range of 150Hz-350Hz but who is only hitting intentionally high targets in the 300-330Hz range%
...
\caption{A recording with an attested f0 range of about 250Hz to 365Hz.%
...
Another case where the labeller may want to use their ear is when the f0 track becomes untrackable or unreliable. For example, in an example like raining-ranges, labellers report being able to hear the pitch continuing to fall, even after \langtextquot{raining}, where the f0 track gets lost and/or becomes unreliable because of issues with voice quality. (i.e., Here the labeller knows that the end of \langtextquot{there} is not higher than \langtextquot{raining}, so the tracked pitch must be erroneous.)

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Ranges-raining-ranges-adv.png}
\caption{The labeller provides an educated guess of the pitch minimum during \langtextquot{out there}.%
\label{fig:raining-ranges Ranges Adv}%
\index{Annotated example, Ranges tier (advanced)!raining-ranges}
}
\end{figure}

Here the f0 measured at the end of \langtextquot{there} is measured as around 200Hz; as this might be an instance of pitch doubling, an educated guess for the final f0 may be 100Hz (as indicated by the comma-override), but the lowest reliable f0 (towards the end of \langtextquot{raining}) is around 170Hz. This leads to the Ranges label of \polarquot{170(100:na)-205}.
...
\end{scope}
\end{tikzpicture}
\caption{A hypothetical f0 track.%
...
{[\textit{min}]-[\textit{max}]} &
	...
	If the local pitch min is 244.2Hz and the local pitch maximum is 381.5Hz, the label should be \polarquot{240-385}
	\tabularnewline
{[\textit{min}]([\textit{min2}]:na)} &
	...
	If the local attestable pitch min is 112, but the labeller intuits that the speaker has deliberately not reached a low \polarquot{110(90:na)-385}
	\tabularnewline
{[\textit{max}]([\textit{max2}]:na)} &
	...
	If the local attestable pitch min is 192, but the labeller intuits that the speaker has deliberately not reached a high \polarquot{90-192(300:na)}
	\tabularnewline
{\polar{X-}[\textit{max}]} &
	...
	If the local pitch max is 264.8Hz and the local pitch maximum is not easy to determine, the label should be \polarquot{X-270}
	\tabularnewline
{[\textit{min}]\polar{-X}} &
	...
	If the local pitch min is 138.2Hz and the local pitch maximum is not easy to determine, the label should be \polarquot{135-X}
	\tabularnewline
\polar{NA} &
	“NA” indicates a stretch of unreliable f0 tracking, in places where surrounding information cannot be used to infer the pitch range minimum\slash maximum (\textit{To be used in last resort scenarios}) &
	If the pitch is a long stretch of unreliable f0 tracking, the label for that unreliable stretch is \polarquot{NA}
	\tabularnewline
\bottomrule 
\caption[An expanded set of labels for the Ranges tier.]{An expanded set of labels for the Ranges tier. The fundamental label is highlighted, with the remaining labels being for recordings with less clear f0 tracking.}
\end{longtable}
...
Here, \langtextquot{seems like} occurs in a pitch range with a much higher f0 maximum than \langtextquot{it’s out of order again}, but the labeller also perceives \langtext{out} and \langtext{order} as high pitched. Though the pitch range changes after \langtextquot{seems like}, the labeller does not perceive enough of the cues to disjuncture (e.g., pauses, final lengthening, initial strengthening, etc.) for them to label different prosodic phrases on the PrStr tier. As such, the labeller does not annotate a \polarquot{]} between \langtext{like} and \langtext{it’s}, but does annotate a new Ranges boundary with a lower ceiling, to indicate that they hear \langtext{out} and \langtext{order} as high accents.
...
\noindent3) \uline{Ambiguity between H and L targets}:  There are circumstances in which it is clear that a tonal target for a boundary or a prominence has occurred, but it is difficult to determine whether that target is a H or a L tone.  ToBI labellers are familiar with the uncertainty\slash ambiguity that arises when a pitch accent in a very compressed low range, often at the end of a phrase, might be a Low (\tobi{L*}) or a downstepped High (\tobi{!H*}).  Similarly, sometimes an \tobi{L*+H} label is required for a pitch accent even though the \tobi{L*} accented syllable is realized with a higher f0 than the preceding syllable.  PoLaR does not explicitly distinguish between H and L target tones, but rather uses the Range annotation and f0 values to determine which of 5 levels each target is realized with.  As a result, it does not require labellers to make a phonological decision about H vs L, but instead provides the option of noting the compressed (or expanded) pitch range.
...
\paragraph{Advanced Levels Example 2: Downstep or Declination}

The issue of distinguishing among local Ranges within an utterance or across utterances can often be challenging, but it is made especially challenging in the context of pitch declination (due to phonological downstep and/or phonetic downdrift). For example, when a pitch accent follows a sequence of pitch range compressions (i.e., after a string of ToBI \tobi{!H*}s), it may be realized with a rather low f0 but it is not always clear whether there has been another application of pitch compression and it is high in its range (i.e., it is another ToBI \tobi{!H*}) or whether it is low in its range (i.e., it is a ToBI \tobi{L*}). While PoLaR does not make this intuition easier to access, PoLaR encourages the labeller to evaluate this question consciously, through the Levels and Ranges labels, and does not provide a simple binary choice of H or L.  As an example, consider the different Levels labels that emerge based on the different Range labels in the figures below.
...
If there is no additional range compression in the \langtextquot{cities} portion of the utterance (i.e., there is a single Ranges interval for \langtextquot{iraqi cities}), then the Levels label for the tonal target in the accented vowel comes out as a 3. If the labeller includes a final range compression for \langtextquot{cities}, then the same Levels label comes out as a 5. Ultimately, the labeller can use these different Levels values to inform whether they think the more appropriate Ranges labels (given their intuition about pitch height) includes this final pitch range compression or not. A labeller may also use their knowledge of intonational phonetics\slash phonology to inform which labels they prefer. Particularly, a labeller who believes the final pitch accent is simply a phonological High that has undergone phonetic declination might prefer the labels on the left, whereas a labeller who believes the final pitch accent is a downstepped phonological High might prefer the labels on the right. A different labeller might not analyze downstep or declination here at all, and might prefer Ranges labels where the Levels label in the accented vowel of \langtextquot{cities} comes out as a 1 or a 2 (as in, e.g., labels where there is just one Range of 120-220 for this entire portion of the recording).
...
A labeller may hear this recording as consisting of a high peak on \langtextquot{hey} falling to a mid or mid-high plateau for the remainder of the utterance. As such, the attested f0 minimum is around 250Hz, but this f0 minimum does not reflect low pitch. For this reason, the labeller can adjust the pitch minimum with an \polarquot{:na} label as done in this figure (cf. section \ref{sec:some-trickier-cases-with-local-pitch-ranges}). When it comes to identifying an appropriate value in the parentheses (here: \polarquot{135:na}), a labeller can put a guess and run the Levels labeller script, iteratively, adjusting the parentheses value and running the Levels labeller, until the Levels value reflect the labeller’s intuition (here: that it is a mid-range pitch, i.e., a Levels value of 3).\\
...
\textit{How should the Range be divided up algorithmically into levels?} Currently, as noted above, the local Range is divided into five equal quintiles. We anticipate that this algorithm may need adjustment, as more discoveries are made. For example, this number of divisions might be either too small or too large, or it might be inappropriate to treat these levels as equally-sized, in terms of pitch space. There are many plausible alternatives to this approach. For example, one alternative might be to vary the width of the quintiles so that the middle section of the current Range, designated as \polarquot{3}, is wider than \polarquot{1} and \polarquot{5}, or vice versa. Another possible algorithm might divide the pitch space into ten levels. Because the Levels-labelling algorithm is easy to adjust (via editing the PoLaR plugin scripts), such adjustments should be made as more empirical findings emerge (perhaps on the basis of studies that use Levels labels) about the most appropriate approach.
...
For certain goals and datasets, it may be useful to systematically label more specific analyses. To allow this flexibility, while also remaining consistent to enable comparison across labellers, an ‘analysis tag’ —introduced with a \polarquot{\#} delimiter— can be added to an existing PoLaR labelling tier.

\begin{longtable}{cp{.8\linewidth}} \toprule \textbf{Tag} & \textbf{Significance} \tabularnewline
\midrule \endhead
\polar{\#X} & The PoLaR label to which this tag has been added is being analyzed as having property X (where X is a variable). \tabularnewline
\bottomrule
\caption[A proposal for a means to encode additional grammatical analyses.]{A proposal for a means to encode additional grammatical analyses. (Note that these \polarquot{\#X} tags can be stacked, to create a PrStr label such as \polarquot{]\#X\#Y\#Z}, indicating a perceived boundary that has characteristics X, Y and Z.)}
\end{longtable}

For example, a labeller who would like to distinguish nuclear stress prominences can augment the PrStr \polarquot{*} label to create \polarquot{*\#NS}. Below we discuss several particular cases where we anticipate that such ‘analysis tags’ may be useful.
...
A labeller who adheres to this (or some other) prosodic hierarchy may wish to keep track of particular prosodic boundaries in PoLaR labels, instead of the perceptually (and not phonologically) defined labels like \polarquot{]} (or \polarquot{[}, \polarquot{]]}, and \polarquot{[[}). This can be achieved with analysis tags like \polarquot{\#IP}, to create PrStr annotations like \polarquot{]\#IP}, to indicate the right edge of an intonation phrase.

These sorts of tags may be especially useful for languages that make use of more than two phrasal levels of prosodic hierarchy (e.g., a labeller of Korean may wish to use \polarquot{]\#AP}, \polarquot{]\#ip}, and \polarquot{]\#IP}; see \citealt{jun07}), or for languages\slash varieties\slash data whose prosodic hierarchy is less clear but for which more than two hierarchical elements can be identified. A labeller of such data where the phonological analysis is still uncertain may wish to use labels like \polarquot{]\#A}, \polarquot{]\#B}, \polarquot{]\#C}, etc., to remain agnostic about particular analyses.

Two examples are given below. The first is an example from MAE\_ToBI training materials (see also \citealt{beckman-05}). In this Advanced PoLaR annotation, each \polarquot{]} on the PrStr tier is annotated with MAE\_ToBI’s numerical labels for constituent categories in the prosodic hierarchy: \polarquot{\#2}, \polarquot{\#3}, and \polarquot{\#4}.
...
The second example comes from Korean.  In this example, each \polarquot{]} on the PrStr tier is marked with one of the boundary types in the analysis proposed in \citealt{jun07}, including \polarquot{]\#AP}, \polarquot{]\#ip}, and \polarquot{]\#IP}.
...
\caption[The labeller analyzes the first phrase boundary as lower on the hierarchy than the boundary at the end.]{\texttt{breathalyzer}: This is the second half of this recording; the labeller analyzes the first phrase boundary as lower on the hierarchy than the boundary at the end.%
\label{fig:breathalyzer Beyond}%
\index{Annotated example, Beyond the guidelines!breathalyzer}
}
\end{figure}

\subsection{Contrastive Focus and Prosodic Events}\label{sec:contrastive-focus-prosody}

In terms of semantic interpretation, contrastive focus is often treated as a grammatical primitive, resulting in the calculation of semantic alternatives (see, e.g., \citealt{rooth92}, \citealt{truckenbrodt95}, \citealt{wagner05}, \citealt{kratzerselkirk20}). In terms of prosody, a variety of languages have been argued to mark contrastively focused elements with particular prosodic forms. For example, contrastive focus may manifest as a particularly strong prominence, which may have a particular intonational form (\citealt{pierrehumberthirschberg90} for American English, \citealt{baltazanijun99} for Greek, etc.) or which may interact in particular ways with surrounding prosodic structure (\citealt{ishihara07} for Japanese, \citealt{kanerva90} for Chichewa, etc.). Taking this idea into account, labellers may wish to use an optional tag, such as \polarquot{\#CF}, to track their analysis of the loci of semantic/pragmatic focus, and to then use such meaning-tracking labels to further analyze the phonetic/phonological realization(s) of such grammatical features. An English example is provided below, where \langtextquot{illusion} is marked with \polarquot{**\#CF} to indicate that the labeller’s analysis that this is the head of semantic/pragmatic focus (as it is the focus associate of the focus-sensitive operator ‘only’).

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{PrStr-asked_only_an_illusion-adv.png}
\caption{The labeller has added the “analysis tag” \polarquot{\#CF}, to indicate that the prominence on \langtextquot{illusion} is the focus associate of the semantic/pragmatic focus-sensitive operator \langtextquot{only}, and that this prominence could be a phonological exponent of contrastive focus.%
\label{fig:only_an_illusion Beyond}%
\index{Annotated example, Beyond the guidelines!only\_an\_illusion}
}
\end{figure}
% Reviewer: figure missing #CF
% TODO: add this tag to the ** associated with 'illusion'

% TODO: consider adding an example with 2nd occurrence focus where the #CF tag would be somewhere without clear phonetic realization of L+H*

Contrastive focus in English is analyzed as having a grammatical relationship with phonological prominence (and not with phonological phrase boundaries), as the \polarquot{**\#CF} label notates in Figure \ref{fig:only_an_illusion Beyond} above. However, in other languages, contrastive focus is (also) grammatically related to phrase boundaries (e.g., contrastively focused constituents in Korean are marked with an additional phrase boundary; \citealt{jeonnolan17}); in such languages, the \polarquot{\#CF} tag should (also) be added to the appropriate \polarquot{]} or \polarquot{[} label.
...
\midrule
\endhead
\polar{]\#X} & Phrase Boundary for a Phrase of Type “X” & A prosodic phrase ends at this point; the type of prosodic phrase is specified by “X” (e.g., \polar{]\#IP}) \tabularnewline\hdashline
\polar{[\#X} & Phrase Boundary for a Phrase of Type “X” & A prosodic phrase begins at this point; the type of prosodic phrase is specified by “X” (e.g., \polar{[\#IP}) \tabularnewline\hdashline
\polar{*\#X} & Strongest Prominence in a Domain & The constituent this occurs in is the most prominent in the prosodic domain specified by “X” (e.g., \polar{*\#IP}) \tabularnewline\hdashline
\polar{*\#CF} & Contrastive Focus Prominence & The prominence marks Contrastive Focus meaning \tabularnewline
 \bottomrule 
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item a summary table of the labels (formatted like Table \ref{XXX} and others in this guide)
	\end{enumerate}
...
\end{scope}
\end{tikzpicture}
\caption{The final low plateau and low plateau with an extra fall have conventionally been labelled as \tobi{L-L\%} in MAE\_ToBI.%
...
\end{scope}
\end{tikzpicture}
\caption{A contour with two High pitch accents (MAE\_ToBI: \tobi{H*}) can variably have a plateau between corresponding peaks or various degrees of sagging between them.%
\label{fig:sagging Usages}%
}
\end{figure}

Using PoLaR’s (mostly phonetic) labels alongside phonemic labels like ToBI’s could help explore systematicity in the phonetic variation within these phonemic labels. For example, in the case of Fig. \ref{fig:final low Usages}, PoLaR Points, Levels, and Ranges labels would differ between the two contours, potentially uncovering systematicity in how \tobi{L-L\%} is realized. In the case of Fig. \ref{fig:sagging Usages}, such sagging has been argued to be surface variation of the same phonological sequence of \tobi{H*}s; PoLaR Points and Levels Ranges labels may help to uncover whether there is systematicity about the degree, timing, and depth of such sags.

In addition, such work (or even work where PoLaR labels alone are annotated) may even possibly uncover new phonemic distinctions. For example, there may be phenomena that have been treated as category-internal variation (e.g., \tobi{L-L\%} realized as either a final fall or low plateau, or various degrees of sags between \tobi{H*}s) where the variation turns out to be contextually-determined; and if the relevant contextual variables are semantic\slash pragmatic, this could be analyzed as an undiagnosed phonemic distinction, which in turn could be used to argue for readjusting the intonational phonemic inventory of the language.
...
\begin{itemize}
	\item At what rate do accents like \tobi{L*} or \tobi{H*} get realized with each of the possible Levels values?
	\item How do peaks\slash valleys (as labelled on the Points tier) align with Phone segments that are marked with a pitch accent label like \tobi{H*} or \tobi{L*}?
	\item How are the beginnings of phrase-final boundary plateaus (ToBI \tobi{H-} and \tobi{L-}) timed with respect to Phones segments and/or PrStr \polarquot{*} and \polarquot{]} labels?
	\item etc.
\end{itemize}
...
\end{scope}
\end{tikzpicture}~~\begin{tikzpicture}[scale=.85, transform shape]
\node[rotate=90] at (-0.75,3) {\relsize{-2}f0 (Hz)};
\begin{scope}[anchor=east, style={font=\relsize{-2}}]
	\node at (0,1) {100};
	\node at (0,2.5) {150};
...
The difference between the first and second hypothetical utterances is the f0 scaling: as shown by the scale at the left side of each panel and the range markers at the bottom, the former has a pitch range of 100-400Hz, while the latter has a pitch range of 100-200Hz. In terms of PoLaR, these two utterances would be annotated identically, save for Ranges tier.\footnote{Of course this is not to say the two utterances should be treated as identical intonational realizations, even if the slopes can be measured in a way to make them identical. Indeed, the differences in f0 range may be meaningful (especially if both a produced by a single speaker) — this type of difference has indeed a primary motivation for the existence of the Ranges tier in PoLaR.} In both cases, the two phrase-final Levels labels would be 1 and 5 (a rise from low to high), Occurring .25sec apart, the slope would be 16 Levels/sec. However, if slope were calculated based on absolute f0 values, the two would have very different slopes: the former would have a slope of 1200Hz/sec, and the latter would have a slope of 400Hz/sec. In other words, because of how Levels labels are calculated relative to the pitch range, Levels labels inherently normalize rate of change in pitch. This is not to say that Levels/sec is a better measure of pitch slope —indeed, Hz/sec (or st/sec) may be more appropriate, at least in some cases— but rather that PoLaR Levels offer a straightforward way to compare pitch contours without additional data wrangling.\footnote{To be clear, PoLaR Levels labels are a way of normalizing f0 measures, with guidance from human perception of pitch and pitch ranges. This normalization facilitates comparison of pitch contour slopes across utterances. Because slopes depend on time measures as well, if the two hypothetical utterances depicted in Figure \ref{fig:Usages-slopes} %
% TODO: Byron's note to self: make sure this is the right reference number, when creating the final LaTeX version
are produced at different speech rates, slope calculations using direct measures of time (in, e.g., milliseconds) will produce different values for the final slope. For this reason, slope comparisons across utterances would benefit from  normalizing the dimension of time. Choosing between methods of time normalization depends on the nature of the comparison, and this is beyond the scope of the PoLaR framework.}
...
Using these labels, we could measure slopes like the steepness of the rise between the prominence-related Points (the ones labelled here as \polarquot{*>} and \polarquot{*<}), or the steepness of the fall from the last prominence-related Point and the first phrase-related Point (the ones labelled here as \polarquot{*<} and \polarquot{]>}). As before, these are measures that may be familiar to intonationists, but PoLaR labels facilitate finding the relevant endpoints for the measures of slope.

An additional way in which PoLaR can be helpful for calculating f0 slopes is when it comes to files with poor f0 tracking. It is common for those working on f0 measures to need to exclude some number of recordings because of software errors, recording conditions, or microprosodic effects of segments or voice quality. However, because PoLaR labellers can supply comma-override labels in these contexts, many of these files with f0 tracking errors could still be used. For example, the Figure below exhibits f0 tracking errors during \langtextquot{out there}, during which the pitch is perceived to be falling.
...
Here the f0 at the Points label during \langtextquot{raining} is read as 140.8Hz at 0.547sec, with the final Points label is read as 65Hz (from the comma-override) at 1.290sec – a slope of approximately ​​-102Hz/sec (or in terms of Levels, approximately -4 Levels/sec). Thus comma overrides can be used as a way to include recordings that might otherwise need to be excluded.
...
\begin{infobox}[frametitle=\textbf{Generating “Pseudo-Categorical” Labels from PoLaR Labels}]
	...
	Pseudo-categorical labels such as these can be especially useful in keeping track of the distribution of allophones, as described earlier in this section. In the examples above, assuming each instance of “librivox” is labelled \ipaslashes{\tobi{L+H*}}, we could say that \ipaslashes{\tobi{L+H*}} has allophones of \ipabrackets{\polar{2+4*}} and \ipabrackets{\polar{4+*+5}}.
	
	Of course using PoLaR labels to construct the boundaries of a category requires an algorithm. A preliminary attempt at implementing such an algorithm is coded into a script in the PoLaR plugin for Praat. The basics of this algorithm are as follows. We begin with prominence-associated labels. The script identifies the Phones-tier segments during which a \polarquot{*} label occurs and any Points tier labels that are associated with that \polarquot{*} (through \polarquot{<}, \polarquot{>}, or \polarquot{@} labels in the Advanced Points labels). For every \polarquot{*}-associated Points label within the relevant Phones interval, the corresponding Levels label appears next to the \polarquot{*} in the Pseudo tier label. For \polarquot{*}-associated Points labels that precede the relevant Phones interval, the associated Levels label(s) is/are inserted, followed by a \polarquot{+} (akin to ToBI complex tones). For example, during \langtextquot{librivox} in \texttt{librivox\_recording-1}, there is a \polarquot{2+4*} label indicating that the Level 4 Point occurs during the \polarquot{*}-marked segment, and it is preceded by a Level 2 Point that occurs before the \polarquot{*}-marked segment. On the other hand, in \texttt{librivox\_recording-2}, a \polarquot{4+*+5} label occurs within \langtextquot{librivox}, indicating that neither Points label occurs within the \polarquot{*}-marked Phone segment, and that a Level 4 Point occurs before the segment, and a Level 5 Point follows it. 

	As for boundary-associated labels, the script identifies the Phones-tier segments after which a \polarquot{]} label occurs and any Points tier labels that are associated with that \polarquot{]}. For every \polarquot{]}-associated Points label within or after the relevant Phones interval, the corresponding Levels label appears before the \polarquot{\%} in the Pseudo tier label. For \polarquot{]}-associated Points labels that precede the relevant Phones interval, the associated Levels label(s) is/are inserted, followed by a \polarquot{-} (akin to ToBI phrase accents). For example, after \langtextquot{recording} in \texttt{librivox\_recording-1}, there is a \polarquot{1-3\%} label indicating that the Level 3 Point occurs during the \polarquot{]}-marked segment, and it is preceded by a Level 1 Point that occurs before the \polarquot{]}-marked segment. After \langtextquot{librivox} in \texttt{librivox\_recording-1} there is a \polarquot{1-\%} label, indicating that there is a \polarquot{]}-associated Level 1 Point before the phrase-final segment, and no \polarquot{]}-associated Point during the phrase-final segment.
...
Though it is well known that pitch range is dynamic —both over the course of a single utterance and across utterances— less is known about the more precise ways in which they change. Once more precise annotation is kept, we can ask if the different ways in which pitch ranges change correspond to different (linguistic) contexts. There has been some research on the topic of pitch range and its connection to a variety of linguistic disciplines —including prosodic phonology, syntax, semantics, pragmatics, sociolinguistics, and discourse structure— from empirical angles of both production and perception.
...
\caption{%
The different stars indicate perceived alignment of the different types of contours. In the case of the two rises, the overall changes in shape influence the perceived alignment even though the turning points (the beginnings and ends of the rises) remain constant.
...
To exemplify how PoLaR can reduce labeller disagreements and facilitate focused discussion, here we consider some examples of disagreements that often arise in alternative labelling systems that are designed for phonological labelling (e.g., MAE\_ToBI), and consider how working with PoLaR might help. When such a case arises, the labels themselves direct labellers to specific aspects of the signal to discuss. For example, consider the utterance below, where labellers disagree about whether the high pitch during “it” ought to be labelled as \tobi{H*}, as \tobi{\%H}, or not labelled at all.

\begin{figure}[H]
\centering
\includegraphics[width=.875\linewidth]{Usages-surprised_him.png}
\caption{MAE\_ToBI labellers may disagree or be uncertain about whether a label of \tobi{L+H*} or \tobi{H*} is more appropriate for \langtextquot{indeed}. The different possible labels are indicated on the two “ToBI Alt” tiers.%
\label{fig:Usages surprised him}%
\index{Annotated example, Usage cases!surprised\_him}
}
\end{figure}

Is this because of different perceptions with respect to prominence (i.e., \tobi{H*} vs. \tobi{\%H} or no label), or is it because labellers disagree whether the pitch is meaningfully at a high target (i.e., \tobi{H*} or \tobi{\%H} vs. no label)? Instead of forcing annotators to choose an analysis before creating a complete set of labels, PoLaR allows the uncertainty of prominence to be captured as a \polarquot{?*} on the PrStr tier, and the lack of commitment to a specific phonological analysis is captured as a \polarquot{0} on the Points tier. For labellers who are more certain, the source of perceptual\slash analytical differences will be easier to identify and discuss, with PoLaR labels that pull apart the relevant issues. In this specific case, the perceptual issue of prominence will be clear through PrStr labels for \langtextquot{it} (i.e., \polarquot{*} vs. \polarquot{?*} vs. nothing), and the analytical issues can be captured as Advanced Points labels (i.e., \polarquot{<[} vs. \polarquot{<*} vs. \polarquot{0}).

As a second example, labellers quite commonly disagree over —or are uncertain about— whether a particular label ought to be \tobi{L+H*} or \tobi{H*}, as below.
...
This disagreement or uncertainty in phonological labels may be because the labellers have different criteria for what constitutes an \tobi{H*} vs. \tobi{L+H*}, or it could be that they differ in their perception of which phonological category is expressed, or both. On the other hand, because Points and PrStr are labelled separately, the PoLaR labels can indicate which aspects of the signal were perceived differently by different labellers. For example, in Figure \ref{fig:Usages and indeed}, this PoLaR labeller analyzes the lower f0 at the beginning of the vowel as associated with the prominence (using the Advanced \polarquot{*>} label on the Points tier), but another labeller might leave this as a \polarquot{0} label indicating they are not analyzing it as associated with the prominence. This information will be useful in focusing discussions about which phonological label (if any) is most appropriate.

In sum, PoLaR labels can lead to more fruitful discussions about how aspects of the signal map onto categorical labels, making it possible to identify some of the issues with categorical labels related to how different labellers interpret the signal differently (unbeknownst to themselves and/or each other). In addition, if PoLaR labellers want to include MAE\_ToBI labelling, the PoLaR labels can be used as a guide.  For example, a decision could be made about which labels ought to be \tobi{H*} and which should be \tobi{L+H*}, e.g., by following an algorithm based on the time-alignment of Points with respect to syllable boundaries, and/or pitch-scaling of Levels.


\subsection{Unlabellable Cases Minimized by PoLaR’s Design}\label{sec:difficultunlabellable-cases}
One of the goals of (phonological) grammars is to predict what sorts of expressions are possible and which are impossible. Being able to accurately make such predictions is the marker of a successful phonological analysis, and many prosodists and intonationists have the goal of understanding where the boundaries of wellformedness lie, and thus make proposals to that end. Some of these proposals constrain systems of prosodic annotation, sometimes to the extent that the annotation system is unable to transcribe a signal that does not conform to the proposed grammar. This issue —the existence of what we call “unlabellable” cases— is one that such systems cannot be used to address, without serious modification.
...
\caption{A steady rise to a peak in the final (unstressed) vowel of \langtextquot{menu}, which is not perceived as prominent or at a phrase boundary.%
...
\caption{A steady high through the final vowel of \langtextquot{number}, which is not perceived as prominent or at a phrase boundary.%
...
\midrule \endhead
\polar{1} & within the lowest 20\% \tabularnewline
\polar{2} & within the 20\%--40\% range \tabularnewline
\polar{3} & within the 40\%--60\% range \tabularnewline
\polar{4} & within the 60\%--80\% range \tabularnewline
\polar{5} & within the highest 20\% \tabularnewline
\bottomrule 
...
The rows \tikz[baseline, inner sep = 0pt, outer sep = 0pt]\node[anchor=base, fill=green]{\textbf{highlighted in green}\strut}; can be considered the ``default'' labels for this tier; all others beyond this mark some kind of intuition\slash analysis on the part of the labeller.
...
\polar{NA} &
	“NA” indicates a stretch of unreliable f0 tracking, in places where surrounding information cannot be used to infer the pitch range minimum\slash maximum (\textit{To be used in last resort scenarios}) &
	For a long stretch of unreliable f0 tracking, the label for that stretch is \polarquot{NA}
	\tabularnewline
\bottomrule 
