...
In addition to being formed of words, spoken utterances contain a wide range of other information about timing, intonation, prominence, phrasing, voice quality, rhythm, etc., often collectively called spoken prosody. (See Ladd 2008, Beckman & Venditti 2011, and Barnes & Shattuck-Hufnagel _forthcoming_ for some broad overviews.) These aspects of an utterance are sometimes called supra-segmental, because they can span regions larger than a single phonemic segment (i.e., a single consonant or vowel). (See Lehiste 1970 for extensive discussion.)

In a language like English, two major categories of prosodic structure concern *prominence* (related to notions of accent, stress, focus, emphasis, etc.) and *phrasing* (related to notions of grouping, disjuncture, pauses, etc.). In turn, both prominence and phrasing correlate with changes in *pitch* (related to notions of f0, tone, intonation, etc.). Speakers of English modulate these and other prosodic aspects of speech and thereby signal distinctive pragmatic, semantic, syntactic, or morphological information. In order to study these phenomena, linguists and speech scientists of many types are interested in annotating the prosodic structure of utterances.
...
Though prominence and phrasing are abstract concepts, manipulation of the intonational acoustics of an utterance can provide strong cues as to which elements are prominent and where phrase boundaries exist.#footnote[The concept of prominence has been defined in a variety of ways, as required by different disciplines. For further discussion see Gussenhoven (2015), Wagner et al. (2015).] A particularly strong set of cues comes from changes in perceived pitch that are caused by changes in the frequency of vibration of the vocal folds (this vibration rate is often called "f0", for "fundamental frequency").#footnote[Note that the cues to phrasing and prominence are _by no means_ restricted to the acoustics of f0. Speakers also manipulate dimensions such as duration, amplitude and voice quality (phonation quality) to signal prosodic structure. For some further discussion, see section \@sec:labelling-individual-cues.] In terms of the meaning of a sentence, intonational differences can play a key role (as exemplified by sentences 1 and 2 above).
...
#figure(
  table(
    ...
    [*pitch*], [an abstract psycho-perceptual phenomenon related to f0 (_requires a listener with a mind_)],
    table.hline(stroke: (dash: "dashed")),
    [*f0 contour*], [a description of changes in the f0 values in an utterance over time],
    table.hline(stroke: (dash: "dashed")),
    [*pitch contour*], [a description of changes in pitch over time (_represents events in a listener's mind_)],
    table.hline(stroke: (dash: "dashed")),
    [*intonational contour*], [an abstract sequence of pitch events over time (_requires a grammar_)],
    table.hline(stroke: (dash: "dashed")),
...
(For further description of past prosodic models and annotation systems, please see, e.g., Roach 1994, Ladd 2008 Chapters 1 and 2, Féry 2017 Chapter 5, Barnes & Shattuck-Hufnagel _forthcoming_.) That said, #underline[*no* familiarity with these systems is required in order to learn and apply the basic aspects of PoLaR annotation].
...
In particular, the present authors have been involved in the development, instruction, and maintenance of the MAE_ToBI system (_M_ainstream _A_merican #underline[E]nglish #underline[To]nes and #underline[B]reak #underline[I]ndices; Beckman & Hirschberg 1994, Beckman & Ayers 1997, Beckman et al. 2005 currently embodied in MIT's Open Courseware system [link]). While committed to the development of PoLaR, the authors remain interested and invested in ToBI annotation systems for labelling phonological categories; we believe the systems are complementary, and not in competition. ToBI is a phonological annotation system, for transcribing intonational categories. It was developed within the framework of AM (#underline[A]utosegmental-#underline[M]etrical) phonology (as in Pierrehumbert 1980, Ladd 2008, Arvaniti & Fletcher 2020), which distinguishes different levels of prosodic phrases (e.g., "Intermediate Phrases" and "Intonation Phrases"), as well as different types of pitch movements (those associated with stressed syllables [e.g., "Pitch Accents"] and those associated with prosodic phrase edges [e.g., "Phrase Accents and Boundary Tones"]). While PoLaR can distinguish such objects, it doesn't require that its labellers commit to any particular phonological analyses. In this sense it contrasts with ToBI, in which all phonological categories of pitch are annotated as either categorically high (H) or low (L), following Pierrehumbert (1980).
...
*Compatibility with other annotation systems / prosodic analyses:*
PoLaR works well with other labelling tools and systems which have different goals, and its use alongside other annotation systems is encouraged; PoLaR is not intended as a complete model of spoken prosody. For example, parallel PoLaR and e.g. ToBI#footnote[ToBI annotation systems exist for a number of languages and varieties; see Jun (2005, 2014) for works describing ToBI systems for a number of languages.] labels can be expected to shed light on both the phonemic inventory of a language and the phonetics-phonology interface.
...
*Expandability of the annotation system:*
Although this monograph focuses on the _intonational_ aspects of spoken prosody, a critical feature of PoLaR's design is that it does not restrict labellers to only annotating this information. To be clear, we mean that PoLaR annotation is broadly intended as a _*framework*_ (which is to say it is a way of conceptualizing annotation systems), so that there is not a rigid way for PoLaR annotation to be implemented.
...
*Crosslinguistic usability:*
PoLaR is designed to be usable for any language as well as for any speech style, register, or dialect _within_ a language. PoLaR's focus on acoustic cues and broad phonological categories makes PoLaR useful in the initial stages of exploring a language or dialect whose prosody is under-/un-documented. While some tiers require native speaker intuition (e.g., prominences, boundaries). others could be used without native speaker intuitions (e.g., turning points). Thus, using PoLaR can be a step toward formulating a phonological transcription system of intonationally-undocumented systems. This is critically important, because our current understanding of human speech prosody is based on analysis of a strikingly small proportion of the world's languages, while at the same time, it has become easier to create corpora of recorded utterances in understudied languages for analysis. Thus the time is right for development of a system such as PoLaR which facilitates getting a foothold on the analysis ladder for addressing a new language or dialect.

Beyond work on understudied languages/dialects, PoLaR can also be useful for studying how suprasegmental cues are used differently across varieties and contexts, within a language/variety. Researchers working with a corpus that is annotated for demographic or contextual information could use PoLaR labels in much the same way as it would be used for crosslinguistic work.#footnote[For those wishing to create their own corpus, see, e.g., Meyerhoff et al. (2011) or Podesva & Sharma (2013) for some discussion of recording this sort of information as well as a description of some best practices.]
...
*How much experience is required to become a user?* PoLaR has been designed to facilitate the labelling process, and to be useful for a wide range of individuals with different levels of experience, backgrounds, and goals. It is designed to require minimal theoretical knowledge, enabling #underline[novice labellers] to begin producing useful annotations more quickly. It is also designed for research groups where different individuals label different aspects of the signal, #underline[in parallel]. Lastly, advanced aspects of the system are designed to be useful for #underline[advanced labellers] or #underline[theoreticians], who are interested in the finer details of the prosody.
...
// Bibliography would go here - for now just a placeholder
// References:
// [All the references from the original LaTeX document would be formatted for Typst here]

// Note: The bibliography section would need to be properly formatted with Typst's bibliography system
// using a .bib file or manual entries. For brevity, I'm not including the full bibliography here.