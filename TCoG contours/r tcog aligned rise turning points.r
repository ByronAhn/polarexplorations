...
# set up functions for calculations
{
    ...
    dome <- function(x) {
        ...
        # out <- A*(sin(B*(x-C)))+D
        # # out <- ifelse(x < 0, 0, ifelse(x>1, sla(x), out))
        # out <- ifelse(x < 0, 0, ifelse(x>=1, 1, out))
        out <- ifelse(
            # x<=.24426, 
            # .22*sin(9.6225*(x-pi/19.22))+.22,
            x<0,
            0,
            ifelse(
                x<=.195747,
                .25*sin(7*(x-pi/14))+.25,
                sin(1.7025*(x-(1-pi/3.405)))
            )
            )
        return(out)
    }
...
tcogplot
ggsave("tcog-all3.png", width = 15, height = 10.2)
